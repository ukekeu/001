...
// Source File Name:   SelectSLB.java
...
public class SelectSLB
{
    ...
    public static String SelectSecBRoom(String s, String s1, String s2)
        throws Exception
    {
        String s3 = "select count(*) counts from room";
        String s4 = "select  SectionNo,Region from roomno group by SectionNo,Region";
        ...
        {
            String s8;
            for(ResultSet resultset = sdatabase.executeQuery(s4); resultset.next(); arraylist1.add(s8))
            {
                i++;
                String s6 = resultset.getString("SectionNo").trim();
                s8 = resultset.getString("Region").trim();
                arraylist.add(s6);
            }
        ...
        String s10 = "select seccode,building,room_code from room order by seccode,building,room_code";
        int k = 0;
        String s11 = "";
        String s13 = "";
        String s15 = "";
        try
        {
            ResultSet resultset1 = sdatabase.executeQuery(s3);
            if(resultset1.next())
        ...
        try
        {
            ResultSet resultset2 = sdatabase.executeQuery(s10);
            for(int l = 0; l < k && resultset2.next(); l++)
            {
                String s12 = resultset2.getString("seccode").trim();
                String s14 = resultset2.getString("building").trim();
                String s16 = resultset2.getString("room_code").trim();
