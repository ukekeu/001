...
%>
 ...
 <tr> 
   ...
   <td class="<%if(sel.equals("2")){out.print("menuselect3");}else{out.print("selemenu4");}%>" align="center" width=88 height=21>
	  <a href="#" onclick="document.form1.sel.value=2;findSub('visitcusview7.jsp');"> <font color='<%if(sel.equals("2")){out.print("#FFFFFF");}else{out.print("#000000");}%>'>诚意客户 
	</td>
	</tr>
</table>
<p align=center> <b><font size="3">诚意客户一览</b> </p>
...
String Cortrol=checke_qx(ViewBean,(String)session.getAttribute("loginid"),"QT11");
    ...
    %>
	<td><input name="choos1" type="radio" value=2 <%if (choos.equals("2"))out.print("checked"); %> onclick="zh();">全部&nbsp;<input name="choos1" type="radio" value=1 <%if (choos.equals("1"))out.print("checked"); %> onclick="zh();">已收诚意金<input name="choos1" type="radio" value=0 <%if (choos.equals("0"))out.print("checked"); %> onclick="zh();">未收诚意金
</td>
...
<input type=button name=modi value="分析" onclick="fx();">
  
    <%}   %>

 
   <input type=button name=sms value="短 信"   title="选择客户后,发送短信给客户"  onclick="sendmessage();">
  ...
  <tr  bgcolor="efefef" >
                        <td height="25" align="center"><input type=checkbox name=ch onclick="chhoseData(this.checked);">序号</td>
                      ...
                      <tr id=TR<%= i %> title="双击弹出窗口修改客户资料" align=center   onmouseover="changCur(this)" onclick="savepara('<%= REF_ID %>','<%=REF_name %>','<%=REF_IF_ID_CARD %>','<%=REF_TYPE %>','<%=REF_SOURCE %>','<%=cusno %>');changeclass1(this,'clickbg','');" ondblclick="window.open('editcustomer.jsp?cusno=<%=cusno %>','edit_i','height=550,width=510,left=150,top=10,resizable=yes,scrollbars=yes,toolbar=no,location=no,directories=no,status=no,menubar=no');"> 
                        ...
                        <td ><%= REF_IF_ACC %>&nbsp;</td>
						  ...
						  <td  align="center"><a href="#"  onclick="savepara('<%=cusno%>');Edit('<%=cusno%>');"><%if (gzState){ %><font color=red>记录</font><%}else{ %>记录<%} %></a> </td>
