...
<%String SQLa="select * from LsCode where type=1";
ResultSet rs=null;
  rs=ViewBean.executeQuery(SQLa);	 %>
<table width="100%" border="0" cellspacing="1"  cellpadding="1" > 
  <tr align="center">
  <td width="76" class=roomleft>反馈信息</td>
  <td class=roomright><table><tr>
 <% while (rs.next()){ %>
 
   
    <td  ><%=rs.getString("codevalue") %> <input name="codes" type="checkbox"  value="<%=rs.getString("codevalue")%>"></td>
...
String Cortrol=checke_qx(ViewBean,(String)session.getAttribute("loginid"),"C002");
   if (Cortrol==null)Cortrol="";
     %>
	  <%if (Cortrol.indexOf("A")>=0){if (serialNo.equals("")){ %>
            <input type="button" value="转为诚意客户" name="tocy" onclick="window.location='../sale/chengyi1.jsp?cusno=<%=StrCusNo %>'"><%}else{out.print("此客户已转为诚意客户 ");%><a href='#' onclick="window.open('../sale/chengyi_edit.jsp?ref_id=<%=id %>','edit_i','height=550,width=500,left=150,top=10,resizable=yes,scrollbars=yes,toolbar=no,location=no,directories=no,status=no,menubar=no');"> 查看</a><br><br><%} %>&nbsp;&nbsp;<input type="submit" value="提　交" name="submit"  >
			<%} %>
            &nbsp;&nbsp;&nbsp;&nbsp;&nbsp; 
...
<tr align="center"><td height=16   width="15%" >&nbsp;跟踪日期</td><td height=16   width="30%" >反馈内容</td><td height=16   width="130%">备注</td><td height=16   width="15%">跟踪人</td></tr>

<%
String SQL="SELECT date,fc,seller,info FROM tail_after WHERE cusno='"+StrCusNo+"'";
 rs2=ViewBean.executeQuery(SQL);
while (rs2.next()){
%>
<tr><td height=16  width="15%" align="center">&nbsp;<%=rs2.getDate("date").toString() %></td><td height=16  width="30%"><%=getbyte(rs2.getString("info")) %>&nbsp;</td><td height=16  width="30%" align="center">&nbsp;<%=getbyte(rs2.getString("fc")) %></td><td height=16  width="15%" align="center">&nbsp;<%=getbyte(rs2.getString("seller")) %></td></tr>
