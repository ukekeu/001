...
<p align=center class=FontColor><font size=3><b>流失客户信息一览表</b></p>
...
<%  
     ...
     %> </select>
	 &nbsp;&nbsp;<select name="order" onchange="changesub(this.value);"><option value="cus_name">按来访姓名排序</option><option value="visit_date">按来访日期排序</option></select>
	...
	%>
    <input type=button name="setcode" value="设置流失原因选项" onclick="openwin('SetCodeValue.jsp?type=0',300,400,100,100);"><%   if (Cortrol.indexOf("F")>=0){ %><input type=button name=count title="流失原因分析" value="分 析" onclick="if (document.form1.section.value==''){alert('请选择楼盘');}else{openwin('LsCustomerFX.jsp?SecNo='+document.form1.section.options[document.form1.section.selectedIndex].value,550,380,200,50);}"> <input type=button name=prin title="打印/导出" value="打 印" onclick="openwin('auto_select4.jsp?para=1&choose=1',400,300);"><%} %>
    <%   if (Cortrol.indexOf("D")>=0){ %><input type=button name=del value="删 除" onclick="Dele();" title="删除时必须单击删除的客户信息"><%} %> <input type=button name=sms value="机 会"   title="把选择的流失客户转为机会客户"  onclick="JH();">
...
<td  align="left" title="<%=address %>"> <%
	...
	<td  align="center" width="8%" title="选择客户流失原因"><a href="#" onclick="openwin('SetCustomerCodeValue.jsp?customer=<%= no%>',300,300,100,100);">流失原因</a></td>
   </tr>
<%
  }
