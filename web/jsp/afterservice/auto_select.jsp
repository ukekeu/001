  ...
  function Validate(aForm){
	 ...
	 if (valueaa==""){
	  ...
	  // document.frm.ifv.focus();
	   ...
	   if (aa==""){alert("排序的数据项必须作为输出数据项,请把排序的数据项加入到输出列表中");return (false);}
		*/
		return (true);
...
<%if (url.equals("17"))url="rep_ajyjlist.jsp";
  if (url.equals("20"))url="rep_czlist1.jsp";
  else url="rep_czlist.jsp";
  
  
 %>
 <form method="POST" name="rmis" action="<%=url %>?PARA_SQL=<%=PARA_SQL %>" onsubmit="return Validate(this)" target="_blank" >
			<tr><td colspan="6">
 ...
 <input type=submit name=submit1 value="预 览/打 印" > <input type=button name=winclose value="关 闭" onclick="window.close();" >
