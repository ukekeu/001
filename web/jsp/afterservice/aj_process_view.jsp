...
<%       List list1=new LinkedList();//存放栏名
		...
		String c_ba="select a.contractno  from appraisal a ,order_contract  s  where s.code*=a.contractno and s.state<>3 and s.payment1 like '%按揭%' ";
		  ...
		  while (rs.next()){
		   ...
		   list2.clear();
		        ...
		        if (room.equals("")){
			      ...
			      sql="SELECT dataname,code FROM  cq_tj_data   WHERE  type=1   and  sectionno='"+section+"'";
	              taxnum=0;
				  ...
				  out.print("<td>银行签订<br>合同日期</td><td>银行经办人</td><td>已存首期</td><td>送房管<br>局登记</td><td>银行审批</td><td>银行出数</td>");
			      out.print("</tr>");
			      rs1.close();
			     }else{
			    room=getbyte(rs.getString("room"));
			     ...
			     sql="SELECT a.dataname,b.cq_data_code FROM  cq_tj_data  a, cq_y_tjdata b WHERE a.type=1 and b.cq_data_code=a.code and  b.contractno="+code+" and sectionno='"+section+"' order by b.code";
