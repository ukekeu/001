...
<body   oncontextmenu="return false"   topmargin="1"    bgcolor="#FFFFFF" text="#000000" onload="document.frm.mappingunit.focus();" >
...
<input type="radio" value="1" checked onclick="ch_ac(this.value);">已售房间&nbsp;<input type="radio" value="2" onclick="ch_ac(this.value);">未售房间&nbsp;<input type="radio" value="3" onclick="ch_ac(this.value);">所有
<table width="100%" border="0" cellpadding="1" cellspacing="1">
  ...
  <%	
	...
	if (!state.equals("")){
	   ...
	   if (!strWhere.equals("")){strWhere=strWhere+" and SaleState="+state;}
	    ...
	    change2Str = "select Code,unitprice,loft,building,room_no,jzarea,salerprice,contractno,customer from order_contract where maap_state=0 and section ='"+SectionS+"' and Loft='"+Loft+"' and upper(building)='"+building+"' order by floors,room_no" ;//+ building.toUpperCase() + "'";
		...
		i=i+1;
			...
			unitprice= FormatD.getFloat1(change2RS.getFloat("unitprice"),2);
			Loft = getbyte(change2RS.getString("loft"));
			building = getbyte(change2RS.getString("building"));
			room = change2RS.getString("room_no");
		    SaleArea = FormatD.getFloat1(change2RS.getFloat("jzarea"),4);
			sumprice = change2RS.getString("salerprice");
		    contractno = change2RS.getString("contractno");
			customer=change2RS.getString("customer");
 ...
 %>
  ...
  <input type=hidden name="Building<%=i%>" value="<%= building %>" >
   ...
   <td > <input type=text name="sumprice<%=i%>"  readonly    size="8"   value=<%=sumprice.trim() %> ></td>
...
document.frm.mappingUnitPrice<%=i%>.value=parseFloat(document.frm.sumprice<%=i %>.value)/parseFloat(document.frm.MappingArea<%=i%>.value);	
 document.frm.mappingUnitPrice<%=i%>.value=subs(document.frm.mappingUnitPrice<%=i%>.value,2); document.frm.Difference<%=i%>.value=parseFloat(document.frm.MappingArea<%=i%>.value)-parseFloat(document.frm.SaleArea<%=i%>.value);
	  ...
	  if (parseFloat(diff.substring(diff.indexOf("-")+1))>parseFloat(percent)){
