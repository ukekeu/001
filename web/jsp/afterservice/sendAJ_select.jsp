...
function submitChoose(){
   document.frm.action="sendAJ_select.jsp?PARA_SQL=<%=PARA_SQL %>";
   document.frm.submit();
}
...
<body onload="setSelectMad(document.frm.out3,'<%=out3 %>');full()">
     ...
     <p align="center"><b><font size="3" color="#000080">请选择送办条件</font></b>
<br>
...
<%if (url.equals("17"))url="rep_ajyjlist.jsp";
  else if (url.equals("18"))url="rep_ajlist.jsp";
  else url="rep_czlist.jsp";
 %>
 <form method="POST" name="rmis" action="rep_ajlist.jsp?PARA_SQL=<%=PARA_SQL %>" onsubmit="return Validate(this)" target="_blank" >
			<tr><td colspan="6">
...
<tr>
  <td width="14%">送办日期</td>
      <td width="18%"><INPUT TYPE="text"  readonly NAME=ajDate1 size=10  onkeydown="key(document.cus.ajDate1)" ><input type=button name=cale value=".." onclick="fPopUpCalendarDlg(ajDate1);return false;" title="请选择按揭日期"></td>
</tr>
 
</table>
 <input type=submit name=submit1 value="确认送办" > <input type=button name=winclose value="关 闭" onclick="window.close();" >
