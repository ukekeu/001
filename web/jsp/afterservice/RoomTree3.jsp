...
<table border="0" width="120%" cellspacing="0" cellpadding="0">
 ...
 <td id="buil_<%=Loft%>"   onclick="changeclassname(this,'BL2','L2');">
  <img src="../images/document1.gif" border=0 align="absmiddle" hspace="4" ><a href="#" onclick="chooseClick('<%=SectionSerialNo %>','<%=Loft %>','<%=SectionName %>','');ChooseForm();aad2(s<%=num %>,200);"   title='单击后在录入测绘的面积' ><%=Loft %></a><img src="../images/h.gif"  name="s<%=num %>"  value="<%=num %>" >
  </td>
  </tr>
   <% 
 ...
 <img src="../images/tree.gif" border=0 align="absmiddle" hspace="4" ><a href="#" onclick="chooseClick('<%=SectionSerialNo %>','<%=Loft %>','<%=SectionName %>','<%=Build %>');ChooseForm();aad2(s<%=num %>,200);"   title='单击后在录入测绘的面积'><%=Build %></a><img src="../images/h.gif"  name="s<%=num %>"  value="<%=num %>" >
...
<script>
 ...
 function ChooseForm(){
     if ( parent.frmright.document.frm.action=="")
      parent.frmright.document.frm.action="AreaMappingA.jsp";
	  if (parent.frmright.document.frm.OK.value=="")
	  parent.frmright.document.frm.action="";
