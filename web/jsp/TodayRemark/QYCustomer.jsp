...
function printTx(){
 ...
 if (document.form1.section.value==""){alert("请选择楼盘名称");return false;}
   openwin1("pQYCustomer.jsp?section="+document.form1.section.value+"&Display="+document.form1.Display.value,750,600,10,10);
 }
  </script>
 ...
 <table BORDER=0 width="100%"  CELLSPACING=1 CELLPADDING=3 ALIGN=center>
  <tr class=TXTColor  >
	 ...
	 <td  align="center" height="23"   >应签约日期</td>
	  ...
	  <td align="center" height="23"   >已首期款</td>
      <td align="center" height="23"   >销售员</td>
	   
    </tr>
	<%} %>
	<tr id=TR<%=i %> align="center"  title="双击可以弹出合同信息" class='listcontent'  ondblclick="openwin1('../sale/showContract.jsp?secname=&secno=<%=sectinno %>&roomno=<%=Room %>&build=&loft=<%=Loft%>&Floor=<%=floors %>&show=true',680,530,50,2);"  onmouseover="mout(this);" onclick="changeclass1(this,'clickbg','listcontent');" > 
     <!--td   ><%=Section%></td>
	 <td   ><%=Loft%></td-->
	<td   ><input type=checkbox name=choos value="<%=sj %>|<%=Section+Loft+Room%>"><%=i%></td> 
      ...
      <td  ><%=Date.substring(0,10)%></td>
	   <td  ><%=Date1.substring(0,10)%></td>
	      <td  align="right"><%=curr+SalePrice%> </td>
	   
		 <td align="right" ><%=yj_money		  %> 
	 </td>
