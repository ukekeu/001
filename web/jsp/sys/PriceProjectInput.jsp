...
<body onload="outdate1();"  oncontextmenu="return false">
...
try{
		for(int j=intBeginR;j<=intEndR;j++){
			...
			loftnum++;
			%>
		     <td align="center" class=listtitle><font color="#ffffff"><%=NO%><br><input type="text" name="unit<%=j%>"   onchange="checkedvalue(this);coun(this.name,this.value);" title="数字" value="0" size=4></font><!--onblur="checkedvalue(this);unitprice('<%=NO%>')" input type=checkbox name="Rcheck1" onclick="check('<%=j%>',this.checked);"--></td>
			 <%}
			 }else if (!Lock.contains(String.valueOf(j))){loftnum++;
			 %>
			 <td align="center" class=listtitle><font color="#ffffff"><%=NO%><br><input type="text" name="unit<%=j%>"   onchange="checkedvalue(this);coun(this.name,this.value);" title="数字" value="0" size=4></font><!--input type=checkbox name="Rcheck1" onclick="check('<%=j %>',this.checked);"--></td>
			 <%} %>	
			 <%}else{loftnum++;%>
			 <td align="center" class=listtitle><font color="#ffffff"><%=NO%><br><input type="text" name="unit<%=j%>"   onchange="checkedvalue(this);coun(this.name,this.value);" title="数字" value="0" size=4></font><!--input type=checkbox name="Rcheck1" onclick="check('<%=j %>',this.checked);"--></td>
	...
	%>
	<tr>
     <td align="center" class=FloorC width="10%"><font color="#ffffff"><div id=A<%=iii+1 %>w1><%=iii%></div><input size=3 type=text name="F<%=Fn %>" onchange="coun(this.name,this.value);"  ><!--input type=checkbox name="pcheck<%=iii %>" onclick="check<%=iii %>();"--></font></td>
		<%
	  ...
	  <%=fontcolor %>
		...
		<%if(state.equals("1")){%>
		<input type="text" size=4 name="P<%=Fn%>A<%=j %>" tab=<%=++tab%>  onblur="checkedvalue(this);" title="数字" value="0"><input type=hidden name="Pr<%=Fn%>A<%=j %>" value="<%=RoomNo%>">
		<%}else{%>
		<input type="hidden" size=4 name="P<%=Fn%>A<%=j %>" tab=<%=++tab%>  onblur="checkedvalue(this);" title="数字" value="0"><input type=hidden name="Pr<%=Fn%>A<%=j %>" value="<%=RoomNo%>">
		<%} %>
	   </td> 
		<%   
        }
  ...
  <script>A<%=iii+1 %>w1.innerHTML="<% if (!floormess.equals("")) out.print(floormess);else out.print(iii);%>";</script>
...
<input type="hidden" name="AllUnitArea_S" value="<%=FormtD.getFloat(AllSumPrice_S/AllSumArea_S,2) %>">
  ...
  <div id="div2" <%if(!page.equals("2")){%>style="display:none"<%}%>>
	...
	<td align="center"  class=listcontent ><%= dda%></td>
     <td align="center" class=FloorC width="10%"><font color="#ffffff"><a href="#" onclick="javascript:openwin('../basic/RoomEdit.jsp?loft=<%=Region %>&seccode=<%=strSection%>&building=<%=strBuilding%>&Floor=<%=dda%>&Room_code=<%=room2%>&MaxFloor=<%=intEndF%>&BengR=<%=no2 %>&EndR=<%=no2 %>&RoomNoType=<%=room2 %>',400,500,150,10);" title="已输入"> <%=room2%></font></td>

	 <td align="center"  class=listcontent ><%= roomtype2%><input type="hidden" name="Qs<%=iii %>" value="<%=roomtype2%>"></td>
	 <td align="center"  class=listcontent ><input type="text" size=4 name="Q<%=iii%>"  onblur="checkedvalue(this);" title="数字" value="0"><input type=hidden name="Qr<%=iii%>" value="<%=room2%>">
	 <td align="center"  class=listcontent >&nbsp;
	 
	 <%}else{%>
	 <input type="hidden" name="Qs<%=iii %>">
	 <input type="hidden" size=4 name="Q<%=iii%>"  onblur="checkedvalue(this);" title="数字" value="0">
	 <input type=hidden name="Qr<%=iii%>" value="<%=room2%>">
	 <%} %>
   ...
   <div id="div3" <%if(!page.equals("3")){%>style="display:none"<%}%>>
	...
	<tr>
	 ...
	 <td align="center"  class=listcontent ><input type="text" size=4 name="R<%=iii%>"  onblur="checkedvalue(this);" title="数字" value="0"><input type=hidden name="Rr<%=iii%>" value="<%=room3%>">
	 <td align="center"  class=listcontent >&nbsp;
	  <%}else{%>
	 <input type="hidden" name="Rs<%=iii %>">
	 <input type="hidden" size=4 name="R<%=iii%>"  onblur="checkedvalue(this);" title="数字" value="0">
	 <input type=hidden name="Rr<%=iii%>" value="<%=room3%>">
	 <%} %>
	...
	<tr>
	 ...
	 <td align="center"  class=listcontent ><input type="text" size=4 name="S<%=iii%>"  onblur="checkedvalue(this);" title="数字" value="0"><input type=hidden name="Sr<%=iii%>" value="<%=room4%>">
	 <td align="center"  class=listcontent >&nbsp;
	  <%}else{%>
	 <input type="hidden" name="Ss<%=iii %>">
	 <input type="hidden" size=4 name="S<%=iii%>"  onblur="checkedvalue(this);" title="数字" value="0">
	 <input type=hidden name="Sr<%=iii%>" value="<%=room4%>">
	 <%} %>
	...
	<tr>
	 ...
	 <td align="center"  class=listcontent ><input type="text" size=4 name="T<%=iii%>"  title="数字" value="0" onchange="ddd('<%=floor5 %>','<%=iii%>')"><input type=hidden name="Tr<%=iii%>" value="<%=room5%>">
	 <td align="center"  class=listcontent >&nbsp;
	  <%}else{%>
