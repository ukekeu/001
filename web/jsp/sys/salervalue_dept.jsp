...
<tr> 
   <td class="<%if(sel.equals("1")){out.print("menuselect3");}else{out.print("selemenu4");}%>" align="center" width=88 height=21>
	  <a href="#" onclick="document.f1.sel.value=1;findSub('salervalue_dept.jsp');"> <font color='<%if(sel.equals("1")){out.print("#FFFFFF");}else{out.print("#000000");}%>'>部门计划 
	</td>
    <td class="<%if(sel.equals("0")){out.print("menuselect3");}else{out.print("selemenu4");}%>" align="center" width=88 height=21>
	  <a href="#" onclick="document.f1.sel.value=0;findSub('salervalue.jsp');"> <font color='<%if(sel.equals("0")){out.print("#FFFFFF");}else{out.print("#000000");}%>'>个人计划  
	</td>
	 </tr>
	 </table>
	<script>
function findSub(para){
 ...
 <table width=100% border="0">
  <tr height="16" bgcolor="#EAEAEA">
   <td align="left">
   <input type=button name=add value="增 加" onclick="if (document.f1.section.value==''){alert('请选择楼盘');return false;}else{openwin1('valueinput_dept.jsp?section='+document.f1.section.value,700,250,100,100);}">
&nbsp;
<input type=button name=modi value="修 改" onclick="Edit();">
   &nbsp;
<input type=button name=modi value="删 除" onclick="Deletes();">
...
<%  
 ...
 <table border=0 width=100% align=center  cellspacing=1 cellpadding=1 >
   ...
   <%  int id=0;
	   ...
	   String QQ=" select * from SalerValue where RentYear ="+RentYear +" and type=1  ";
	   if (!RentMonth.equals(""))QQ+=" AND RentMonth="+RentMonth;
	   QQ+=section;
	 
   ResultSet rs = aBean.executeQuery(QQ +" order by RentYear, RentMonth");
    int u = 0;
   while(rs.next()){
      id= rs.getInt("id");
