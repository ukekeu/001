...
function savevalue(){
		...
		document.frm.action="valueaction.jsp?Operation=Insert&type=0";
		document.frm.submit();
}
...
<%String section=request.getParameter("section");
 ...
 <script language="JavaScript">
   ...
   String sql="select sum(area) as area,sum(sumprice/10000) sumprice,count(*) num  from room where SaleState=1 and seccode='"+section+"'";
	...
	<td class=roomleft >&nbsp;销&nbsp;售&nbsp;员：</td><td  class=roomright ><select name=seller>
<%sql="select seller from seller where section_no='"+section+"'"; 
  
  temprs=aBean.executeQuery(sql);
	 while(temprs.next()){
	   String CODENAME=getbyte(temprs.getString("seller"));
...
<%sql="select count(*) num  from seller where section_no='"+section+"'";  
  ...
  <tr>
    ...
    <td width="18%"  class=roomright>&nbsp;<input type=text name=perarea  size=10 value="<%=per %>" onblur="checkednum(this,document.frm.areavalue.value,'计划面积不能大于可售面积');"  onKeyPress="return(inputformat(this,'1234567890.',event));">M<sup>2</sup></td>
	  <td width="10%"  class=roomleft >&nbsp;计划金额：</td>  
    <td width="18%"  class=roomright>&nbsp;<input type=text name=plansumprice  size=10 value="<%=plan %>"  onblur="checkednum(this,document.frm.sumpricevalue.value,'计划销售额不能大于预售金额');"  onKeyPress="return(inputformat(this,'1234567890.',event));"   > 万元</td><!-- onchange="count1();"-->
 <td width="12%"  class=roomleft >&nbsp;计划套数：</td>  
    <td width="18%"  class=roomright>&nbsp;<input type=text name=planCount  size=6 value="<%=panNum %>" onblur="checkednum(this,document.frm.sumCount.value,'计划销售套数不能大于可售套数');"  onKeyPress="return(inputformat(this,'1234567890.',event));"   > 套</td><!-- onchange="count1();"-->

  </tr>
  
  </table>
  <p align="left">注意：以面三种计划指标可以根据实际情况填写一种</p>
  <p align=center>
