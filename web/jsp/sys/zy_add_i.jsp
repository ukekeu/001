...
function zy_add_a()
{
   ...
   // 	if ( document.forms[0].s2[i22].selected ) {
	...
	if(document.forms[0].s2.length==0)
	   document.forms[0].action_flag.value="del";
	else
	   document.forms[0].action_flag.value="add";
	document.f1.action="zy_add_a.jsp?tid=" + <%= tid%>;
	document.f1.submit();
}
...
<input type="Hidden" name="action_flag" value="">
 ...
 <table border=0 width=100% cellpadding=1 cellspacing=1 class=roomright>
   ...
   <tr>
	...
	<%
	String sql = "select id ,serialno,name from cortrolman"
				 + " where "
				 + " id not in"
				 + " ( select teaman.mid from teaman ) and serialno not in (select sellerid from ManModuleCortrol)" ;
	...
	<td align=left valign=middle>
	 ...
	 <%
	 sql = "select id,name from teaman A,CortrolMan B where A.mid=B.id and A.tid="+tid+"" ;
 ...
 <p align=center>
     <input type=button value="    增加    "  onclick="zy_add_a()">
