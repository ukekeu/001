...
if (Find!=null){
  ...
  //取出条件值
   ...
   if (!section.equals(""))
   strWhere=" and hsm_YLroom.section ='"+section+"'" ;
...
/*
   ...
   CusType=China.getParameter(request,"custype");
   BuyType=China.getParameter(request,"gmyt");
...
String orderSQL="SELECT hsm_YLroom.LOFT2+hsm_YLroom.rooms ROOM  ,CUSTOMER FROM hsm_YLroom ,customer where customer.serialno=hsm_YLroom.customer and hsm_YLroom.section in("+SectionS+")";
 ...
 sql="select Top "+pageSize+"   customer.Code,customer.serialno,customer.cus_name,customer.cus_sex, customer.Phone,customer.sj,customer.Address,customer.Zip_Code ,customer.sj,customer.Address,customer.Zip_Code ,hsm_YLroom.person seller,convert(char(24),customer.visit_date,111) as   visit_date,customer.bbj,customer.sj,hsm_YLroom.dengji Date,section.name,hsm_YLroom.youxiao,hsm_YLroom.section,hsm_YLroom.loft2 loft,hsm_YLroom.rooms room_no from customer,hsm_YLroom,section where  hsm_YLroom.customer=customer.serialno and section.serialno=hsm_YLroom.section and   hsm_YLroom.Section in("+SectionS+") and  customer.Code not in (Select top "+prenum +" customer.code from customer,hsm_YLroom,section where    section.serialno=hsm_YLroom.section and hsm_YLroom.customer=customer.serialno and hsm_YLroom.Section in("+SectionS+") " +strWhere +orderby+")  " + strWhere;
 
 sqlcount="SELECT count(customer.code) code from customer,hsm_YLroom where  hsm_YLroom.customer=customer.serialno and  hsm_YLroom.Section in("+SectionS+")   "+strWhere;
...
try{
    ...
    ViewBean1.executeUpdate("update customer set out='1' where  serialno in (select hsm_YLroom.customer from  customer,hsm_YLroom where  hsm_YLroom.customer=customer.serialno and  hsm_YLroom.Section in("+SectionS+")   "+strWhere+")");
   ...
   String loft="";
   String roomno="";
out.print("<p align=center class=FontColor><font size=3><b>预留客户一览表</b></font></p>");
%>
<script>
function full(){
...
<input type=button name=prin title="打印" value="打 印" onclick="window.open('<%=reportAddrs %>CusListReprot.rpt')">
 <%} %>
...
<table BORDER=0 width="100%"  id=AutoNumber  CELLSPACING=1 CELLPADDING=1 ALIGN=center >
 ...
 <td  align="center"><font color=white>预留单元</font></td>
  <td  align="center"><font color=white>预留日期</font></td>
