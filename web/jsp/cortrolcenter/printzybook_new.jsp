...
<%//if (java.lang.Integer.parseInt(yh)>0){
 ...
 <%} if (!yh.equals("0")){
 %>
 <tr align="center"  ><td colspan="3">额外优惠</td>
  <% 
 ...
 double zcyh=0;
 int  I611=0;
  sql="select * from PayMent where  id in("+paylist+")  and PayName not  like '%按揭%'  order by PayName ";
		rs=ViewBean.executeQuery(sql);
			String PayName="";
		double disc=0;
		String DiscontS1="";
   while (rs.next()){
      String DiscontS=rs.getString("Discont");
  
 %>
 

   
<tr align="center"><td colspan="3"><%=rs.getString("PayName") %>折后总价</td>
   <% 
  
	  DiscontS1=DiscontS;
     StringTokenizer st22 = new StringTokenizer(DiscontS,"*") ;
     int count2 = st22.countTokens() ;
     while (st22.hasMoreElements()) {
       DiscontS=(String)st22.nextElement();
	  if (disc!=0)
       disc= disc*Double.parseDouble(DiscontS)/100.0;
	  else
	   disc= Double.parseDouble(DiscontS)/100.0;
...
if (hyyh==0)hyyh=100;
 zcyh=Double.parseDouble(request.getParameter("dic1"));
    ...
    %>
     <td> <%=fat.getFloat2(Double.parseDouble(sumprice1[jj])*disc*ewyh/100*hyyh/100,0)%></td> 
    <%jj++;
...
<tr align="center"><td colspan="3">折后建筑面积单价(元/M<sup>2</sup>)</td>
  <% 
     ...
     StringTokenizer  A611111 = new StringTokenizer(sumprice,",") ;
     I611 = A611111.countTokens() ; 
     try{
       int jj=0;
      while (A611111.hasMoreElements()) {
        String eee=(String)A611111.nextElement();
       // sumprice1[jj] =eee;
	  
	  
    %>
     <td><%=fat.getFloat2(Double.parseDouble(sumprice1[jj])*disc*ewyh/100*hyyh/100/Double.parseDouble(jzarea1[jj]),0)%></td> 
    <%jj++;
        }
    }catch(Exception s){}%> 
  ...
  <% sql="select * from PayMent where  id in("+paylist+")  and PayName like '%按揭%'  order by PayName ";
		rs=ViewBean.executeQuery(sql);
		  ...
		  disc=0;
   while (rs.next()){ %>
<tr align="center"><td colspan="3"><%=rs.getString("PayName") %>付款折后总价</td>
   <% 
...
<tr align="center"><td colspan="3">折算单价(元/M<sup>2</sup>)</td>
  <% 
    ...
    %>
     <td><%=fat.getFloat2(Double.parseDouble(sumprice1[jj])*disc*ewyh/100*hyyh/100/Double.parseDouble(sy_areas[jj]),0)%></td> 
    <%jj++;
        }
...
for (int arr=0;arr<I1;arr++) {
 ...
 %>
 <td><%=fat.getFloat2(Double.parseDouble(sumprice1[arr])*yh1*hy1/10000*disc/Double.parseDouble(jzarea1[arr]),2)%></td> 
 <%
...
for (int arr=0;arr<I1;arr++) {
 ...
 %>
 <td><%=fat.getFloat2(Double.parseDouble(sumprice1[arr])*yh1*hy1/10000*disc/Double.parseDouble(tnarea1[arr]),2)%></td> 
 <%
...
for (int arr=0;arr<I1;arr++) {
 ...
 %>
 <td><%=fat.getFloat2(Double.parseDouble(sumprice1[arr])*yh1*hy1/10000*disc,0)%></td> 
 <%
...
<%
 ...
 while (rs.next()){
	...
	%>	
	<tr align="center">
	<td width=15%  rowspan="2"><%= PayName%><%//=rs.getInt("Discont") %></td>
...
for (int arr=0;arr<I1;arr++) {
   ...
   // else csnum="80.00";
  double ajk=(Double.parseDouble(sumprice1[arr])*yh1*hy1/10000*disc*Double.parseDouble(csnum)/100);
  int ajm=(int)ajk/10000;
//out.println(ajm);
 %>
 <td> <%=fat.getFloat2(Double.parseDouble(sumprice1[arr])*yh1*hy1/10000*disc-ajm*10000,0)%></td> 
 <%
...
if (yh.indexOf("*")>=0)yh=fat.getFloat3(Double.parseDouble(yh.substring(0,yh.indexOf("*")))*Double.parseDouble(yh.substring(yh.indexOf("*")+1))/100,2);
 ...
 while (year<=30){ %>
	  ...
	  <tr align="center">
	  <%if (year==10){ %>
	<td  rowspan="3" align="center"><%= PayName%><br>(等额本息还款法)</td>
	<%} %>
	<td   colspan="2" ><%=year %>年月供款</td>
	 ...
	 <% 
	  ...
	  ajRS = ViewBean1.executeQuery(ajStr);	
 //平均法
...
for (int arr=0;arr<I1;arr++) {
 //if (arr==0)out.print("<td   >70% </td>");
  double ajcs=Double.parseDouble(csnum)/100;
  //if(Double.parseDouble(jzarea1[arr])>90)ajcs=0.7;
   // else ajcs=0.8;
  Double ajkmoney= (Double.parseDouble(sumprice1[arr])*disc*Double.parseDouble(yh)/100*hy1/100*ajcs);
