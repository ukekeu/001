...
function submitC(){
  document.frm.action="searcher.jsp";
  document.frm.submit();
...
function openwinzy(url,wh,hg,lf,tp) {
 ...
 var todayInMS=today.getTime();
  var newwin=window.open(url,"newwin"+todayInMS,"toolbar=no,location=no,directories=yes,status=yes,menubar=yes,scrollbars=yes,resizable=yes,width="+wh+",height="+hg+",left="+lf+",top="+tp);
  newwin.focus();
  return false;
}
   var sendmess="";
...
<body   topmargin="1" onload="showfull();">
 <form name=frm method="post" action="searcher.jsp">
   <% 
  out.println(saleshow.BeanshowLoft(getbyte1(request.getParameter("section_select")),request.getParameterValues("showLoft"),SectionS)); 
 ...
 out.print(saleshow.BeanCodeShow1(Cortrol,China.getParameter(request,"condi")));
  %> 
  </form> 
  <script>
