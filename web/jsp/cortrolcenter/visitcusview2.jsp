...
String orderSQL="SELECT LOFT+ROOM_NO ROOM  ,CUSTOMER FROM ORDER_CONTRACT where state=1 and section in("+SectionS+")"+strWhere;
 ...
 sql="select Top "+pageSize+"    customer.cus_name, customer.Code,customer.serialno,customer.cus_sex, customer.Phone,customer.sj,customer.Address,customer.Zip_Code ,customer.sj,customer.Address,customer.Zip_Code ,Order_Contract.seller,Order_Contract.floors,convert(char(24),customer.visit_date,111) as   visit_date,customer.bbj,customer.sj,Order_Contract.Date,Order_Contract.sectionname,Order_Contract.section,Order_Contract.loft,Order_Contract.room_no from customer,Order_Contract where  Order_Contract.customer=customer.serialno  and customer.section=Order_Contract.section and Order_Contract.section in("+SectionS+") and  customer.Code not in (Select top "+prenum +" customer.code from customer,Order_Contract where  Order_Contract.customer=customer.serialno and customer.section=Order_Contract.section and Order_Contract.section in("+SectionS+") and Order_Contract.state=1" +strWhere +orderby+")  and  Order_Contract.state=1 " + strWhere;
...
try{
   ...
   String loft="";
   String roomno="";
out.print("<p align=center class=FontColor><font size=3><b>认购客户一览表</b></font></p>");
%>
<script>
function full(){
...
<input type=button name=prin title="打印" value="打印/导出" onclick="window.open('<%=reportAddrs %>CusListReprot.rpt')">
 <%} %>
   <select name=fields onchange="ch(this.value);">
   <option value=cus_name>客户姓名</option>
   <option value=room_no>认购单元</option>
...
<table BORDER=0 width="100%"  id=AutoNumber  CELLSPACING=1 CELLPADDING=1 ALIGN=center >
 ...
 <td  align="center"><font color=white>认购单元</font></td>
  <td  align="center"><font color=white>认购日期</font></td>
