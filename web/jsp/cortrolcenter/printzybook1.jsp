...
<%//if (java.lang.Integer.parseInt(yh)>0){
 ...
 <%//if (java.lang.Integer.parseInt(yh)>0){
  if (!yh.equals("0")){
 %>
 <tr align="center"  ><td colspan="3">尊贵客户额外优惠</td>
  <% 
...
<tr align="center"><td colspan="3">折后建筑面积单价(元/M2)</td>
  <% 
...
<%
 ...
 while (rs.next()){
	...
	%>	
	<tr align="center">
	<td width=6%  rowspan="5">一次置业</td>
	<td width=15%  rowspan="2">一次置业<%//= PayName%><br> 90平方米以内8成按揭<br>90平方米以上7成按揭<%//=rs.getInt("Discont") %></td>
...
if (yh.indexOf("*")>=0)yh=fat.getFloat3(Double.parseDouble(yh.substring(0,yh.indexOf("*")))*Double.parseDouble(yh.substring(yh.indexOf("*")+1))/100,2);
 ...
 while (year<=30){ %>
	  ...
	  <tr align="center">
	  <%if (year==10){ %>
	<td  rowspan="3" align="center">银行按揭月供款<br>(等额本息还款法)</td>
	<%} %>
	<td     ><%=year %>年月供款 </td>
...
<%
 ...
 while (rs.next()){
	...
	%>	
	<tr align="center">
	<td width=6%    rowspan="5">二次或二次以上置业</td>
	<td width=15%  rowspan="2"><%//= PayName%><br> 6成按揭<%//=rs.getInt("Discont") %></td>
...
if (yh.indexOf("*")>=0)yh=fat.getFloat3(Double.parseDouble(yh.substring(0,yh.indexOf("*")))*Double.parseDouble(yh.substring(yh.indexOf("*")+1))/100,2);
 ...
 while (year<=30){ %>
	...
	<td   ><%=year %>年月供款 </td>
	 ...
	 <% 
	    ajStr = "select id,year,MonthTax,yearTax,yong_tu,MonthPay  MonthPay,lx from ajllgkb  where type=3  and yong_tu=1 and year="+year;//where id >=" + String.valueOf(firstPos);
	  ajRS = ViewBean.executeQuery(ajStr);	
 //平均法
   pjpay=0;
	while (ajRS.next()) { 
...
//  if(Double.parseDouble(jzarea1[arr])>90)ajcs=0.7;
 ...
 <%year+=10;}
  ...
  <tr><td>2、折后总价=折前价格×诚意客户额外优惠×尊贵客户额外优惠；</td></tr>
     <tr><td>3、以上诚意客户及尊贵客户额外优惠仅限开盘当天，第二日自动失效；</td></tr>
