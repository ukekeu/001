...
%> 
<div align="center">
  <p><font color="#0000CC"><b><font size="5" color="#000000">选房计价</font></b></font></p>
...
<tr>
	 ...
	 <td align="right" width="80%" >	   
   	   <input type=button name=view2 value="详 情" onclick="view();">
	   <input type=button name=view2 value="看房记录" onclick="reco();">
	   <input type=button name=view2 value="看房分析" onclick="fx();">
	 
	      <input type=button name=view1 value="试 算"   onClick="ChooseRoom();">
   </td>
	</tr>
</table>
  ...
  <table width="90%" border="0" CELLSPACING=1 CELLPADDING=3>
   <tr>
	...
	String Floor="";
	String  sqlR="select RoomNo.RoomNo,Room.salestate state,RoomNo.Floor,RoomNo.room,Room.floormess from RoomNo,room where ";
	...
	<tr>
		<td align="center" class=FloorC><font color="#ffffff"><div id=A<%=String.valueOf(java.lang.Math.abs(i))+1 %>w1><%=i%></div></font></td>
		<%for(int j=intBeginR;j<=intEndR;j++){
       	String RoomNo="";
		String no="";
	   ...
	   if (!LockRoomS.trim().equals(String.valueOf(j))){
		...
		{%>
		<td id="TR<%=i %><%=j %>" align="center" class=roomright onclick="changeclass1(this,'clickbg','listcontent');savepara('<%=strSection%>','<%=Loft%>','<%=RoomNo%>','<%=Floor %>','<%=strBuilding %>');"><a href="#" ><%=RoomNo%></td>
		<%		
		}else{out.print("<td align='center' class=roomright id='TR"+String.valueOf(i)+String.valueOf(j)+"' >&nbsp;</td>");}
    	}
		}
  ...
  function opennewwin1(para,wh,hg,lf,tp) {
   ...
   function savepara(para1,para2,para3,para4,para5){
	...
	function view(){
		   ...
		   else{openwin("../basic/showroomdate.jsp?secno="+secno+"&roomno="+roomno+"&build="+build+"&floor="+Floor+"&show=true&loft="+loft,500,590,100,10); }
	}
	function fx(){
	      openwin("KFfx.jsp?secno=<%=strSection %>",600,450,100,10);  
	}
	
	function reco(){
	       if (secno==""){alert("请选择房间(只需单击列表中某行)");}
		   else{openwin("ld_sale.jsp?secno="+secno+"&roomno="+roomno+"&build="+build+"&floor="+Floor+"&show=true&loft="+loft,690,450,100,10); }
	}
 function Changes(){
  ...
  function LDROOM(){
      ...
      if (secno==""){alert("请选择房间");}
	    else{openwin("../sale/TemporarilyBuy1.jsp?secno="+secno+"&secname="+secname+"&roomno="+roomno+"&build="+build+"&loft="+loft+"&jzarea="+jzarea+"&code="+code,550,400,100,10);}
	 } 
 function AJ(){
       if (secno==""){alert("请选择房间");}
...
function ChooseRoom(){
      if (secno==""){alert("请选择房间");}
	    else{openwin("../sale/ChooseRoomCount.jsp?secno="+secno+"&secname="+secname+"&roomno="+roomno+"&build="+build+"&loft="+loft,550,500,200,100);}
	 } 	 
