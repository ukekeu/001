...
<%
 ...
 <a href="#" title='单击后在右边是销控表' onclick="aad2(s<%=num %>,200);ChooseFind('<%=SectionSerialNo %>','<%=Loft %>','<%=SectionName%>','');"><%=Loft %></a><img src="../images/h.gif"  name="s<%=num %>"  value="<%=num %>" >
 ...
 <img src="../images/tree.gif" border=0 align="absmiddle" hspace="4" ><a href="#" title='单击后在右边是销控表' onclick="aad2(s<%=num %>,200);ChooseFind('<%=SectionSerialNo %>','<%=Loft %>','<%=SectionName%>','<%=Build %>');"><%=Build %></a><img src="../images/h.gif"  name="s<%=num %>"  value="<%=num %>" >
  </td>
  </tr>
  <%if (jjj==0){ %>
    </table> 
...
<script language="JavaScript">
      function ChooseFind(SecNo,Loft,SecName,build){
		 ...
		 parent.frmright.document.form1.Building.value=build;	 
	       ...
	       parent.frmright.document.form1.action="RoomSaleStateShow.jsp";
