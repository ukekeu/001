...
out.print("</script>");
 ...
 RateSql="select SerialNo,contractno from order_contract where  state<>3 and  loft+room_no <>'"+Loft+RoomNo+"' and section='"+request.getParameter("secno")+"'";
...
try{
  ...
  int iii=0;
   while (RateRs.next()){
     out.print("OrderNo["+String.valueOf(iii)+"]="+"\""+getbyte(RateRs.getString("contractno"))+"\""+";");
	 iii++;
      }	 
}catch(Exception s){  out.print(s.getMessage());out.close();}
RateRs.close();
...
function checkedOrderNoExist(){
    ...
    for (var i=0;i<OrderNo.length;i++){
	    if (document.cus.constractno.value==OrderNo[i]){
		 alert("此合同号已经使用");
