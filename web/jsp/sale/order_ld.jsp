...
String ldSQL="select SL_TemporarilyBuy.*,SL_TemporarilyBuy.saleprice/room.area jzunitprice,SL_TemporarilyBuy.saleprice/room.usedarea unitprice,customer.signatory,customer.address from  SL_TemporarilyBuy,customer,room where SL_TemporarilyBuy.state=0 and SL_TemporarilyBuy.roomcode =room.code and customer.serialno=SL_TemporarilyBuy.cusname and  roomcode="+ldroom;
ResultSet ldRs=null;
String ldCusname="";
String ldCusno="";
String ldCuscardid="";
...
String ldsaleprice="",payment="";
String ldDate="",ldAddress="",ldunitprice="0",ldjzunitprice="0";
try{
  ldRs=ViewBean.executeQuery(ldSQL);
  if (ldRs.next()){

      disc=getbyte(ldRs.getString("disc"));
     ...
     ldjzunitprice=Format.getFloat3(ldRs.getDouble("jzunitprice"),0);
     ldunitprice=Format.getFloat3(ldRs.getDouble("unitprice"),0);
	 ldCussj=getbyte(ldRs.getString("sj"));
	 payment=getbyte(ldRs.getString("payment"));
...
<tr> 
  ...
  <table BORDER=0 width="100%" CELLSPACING=0 CELLPADDING=0 ALIGN=center bordercolor="#ffffff" class="tablefill">
	...
	if (cusSerial.equals("null"))	cusSerial ="";
      ...
      <td colspan="2" class=roomright>&nbsp;&nbsp;<input type=text   name="cusname1"  size=30  onkeydown="key(document.cus.carid)" title="如果多个姓名，请用'/'分隔" value="<%= ldCusname%>" onchange="aa();" >
       <input type=hidden name="editnow" value="<%if (ldCusno.equals("")) out.print("2");else out.print("1");%>">
  ...
  function aa(){
...
</script>  <font color=red>*</font> <input type=button name=FindB  value="查找" onclick="FindOption();"> 
         
    </tr>
    <tr> 
      <td width="16%" class=roomleft>&nbsp;身份证号</td>
      <td class=roomright>&nbsp; <input type=text  name="carid" title="如果多个，请用'/'分隔"   size=68   onblur="countAge();" onkeydown="key(document.cus.tel)" value="<%= ldCuscardid%>" >
        　</td>
    </tr><input type=hidden name=age>
    <tr> 
      <td width="16%" class=roomleft>&nbsp;联系电话</td>
      <td class=roomright >&nbsp; <input type=text  name="tel"  size=30 title="如果多个，请用'/'分隔"   onKeyDown="key(document.cus.sj)" value="<%= ldCustel%>">
        手机&nbsp; <input type=text   name="sj"  size=30 title="如果多个，请用'/'分隔"   onKeyDown="key(document.cus.Address)" value="<%=ldCussj %>"> 
      </td>
    </tr>
    <tr> 
      <td width="16%" class=roomleft>&nbsp;联系地址</td>
      <td class=roomright >&nbsp; <input type=text  name="Address"  size=68  value="" onKeyDown="key(document.cus.custype)" value="<%=ldAddress %>"> 
      </td>
    </tr>
    <tr> 
      <td width="16%" class=roomleft >&nbsp;客户类型</td>
      <td  class=roomright>&nbsp; <select name=custype onKeyDown="key(document.cus.memberT)">
   ...
   <option value=1>需要入户</option>
	...
	<tr> 
      ...
      <td class=roomright>&nbsp; <input type=checkbox name=memberT value=1  onKeyDown="key(document.cus.youhui)"  ><input type=text name=memberNO  readonly   maxlength="50"><input type=button name=FindC  value="查找" onclick="FindOption1();"> 
	  </td>
	  </tr>
    <tr> 
      <td class=roomleft>&nbsp;优惠赠送 </td>
      <td class=roomright>&nbsp; <input type=text name=youhui onKeyDown="key(document.cus.note)"  size=50 maxlength="50"> 
   ...
   %>
    ...
    <tr> 
      ...
      <td  class=roomright>&nbsp; <textarea rows="3" onKeyDown="key(document.cus.listlar)" name="note" cols="35"  ></textarea> &nbsp;销售员:&nbsp; <select name="seller"  multiple size=3  onblur="SelectedSeller();" style="width:100">
          <option value="<%=ldseller %>" selected><%=ldseller %></option>
          <option ></option>
          <option ></option>
          <option ></option>
...
<tr> 
  ...
  <table BORDER=2 width="100%" CELLSPACING=0 CELLPADDING=0 ALIGN=center bordercolor="#ffffff" class="tablefill">
    ...
    <tr> 
      ...
      <td width="12%" class=roomright>&nbsp;<input type=text name="rgsno"   size="10"   onChange="toUpper(document.cus.rgsno)" onKeyDown="key(document.cus.cale1)" onBlur="checkedOrderNoExist();" title="只能是英文字母、数字及"-"组成"><input type=button name=adc value=".." title="选择合同号" onclick="openwin('RgNoList.jsp?SecNo=<%=SecNo %>',200,350,100,100);"></td>
      <td width="19%" class=roomleft>&nbsp;认购日期</td>
      <td width="19%" class=roomright>&nbsp;<input type=TEXT value="<%=curyear+"-"+curmonth+"-"+curday %>" name=Date1 size=10 title="请选择认购日期"  readonly onchange="key(document.cus.pay);" onKeyDown="key(document.cus.VisaDate)"><input type=button name=cale1 value=".." onClick="fPopUpCalendarDlg(Date1);return false;" title="请选择认购日期"></td>
     ...
     %>
        ...
        <input type=hidden name=dis size=6 value="<%=disc%>" readonly>
 ...
 -->
	...
	</td><td  >  <input  type="button" name="addlh" title="添加优惠" size=10 value="..." onclick="discOPEN();"> 
	</td></tr>
      <%} %>     
	 ...
	 <!-- <select name=ajcs style="width : 40" disabled onBlur="count1();"  onchange="document.cus.ajyr.focus();">
	   ...
	   <tr><td class=roomleft>&nbsp;按揭银行</td>
	    <td width="18%" class=roomright>&nbsp;<select name=ajbk   style="width : 105">
         <option></option>
		  <%
   ...
   %>
		...
		<td class=roomright>&nbsp;<input type=text name=gjj size=9 value=0 OnBlur="check(this);dkcount1();">(元)</td>
   ...
   <td class=roomleft>&nbsp;商业  贷款</td>
		<td class=roomright>&nbsp;<input type=text name=sydk size=9  value=0 OnBlur="check(this);dkcount2();">(元)</td>
    ...
    <tr> 
      ...
      <td width="19%"    class=roomright  >&nbsp;以定价为准:<input type=checkbox name=bz value=1 <%if (Cortrol.indexOf("C")<0){ %> title="无审核权" disabled <%} %> checked onclick="checkcount();"></td>&nbsp;<td>&nbsp;<input type=text name="dicprice"  defaultValue=""  value=<%=ldsaleprice %>  size="10"     onblur="check(this);ykj_count();"    <%if (Cortrol.indexOf("C")==-1){ %>   readonly<%} %> >元
	  ...
	  <td width="19%" class=roomleft>&nbsp;首&nbsp;期&nbsp;款</td>
        <td width="19%" class=roomright >&nbsp;<input  type=text size=9  value ="0"   name=firstMoney disabled  onblur="check(this);countsq();dkcount1();">(元)</td>
      
     <input type=hidden name="Adicprice"   value=<%=ldsaleprice %>   size="10" <%if (!(checksumprice>0)){ %>   onblur="check(this);charges();"     <%}else{out.print("readonly title='已有定价，不能直接填写'");} %> onkeydown="key(document.cus.cjjzprice);"> 
   ...
   <tr>
      ...
      <td width="12%" class=roomright >&nbsp;<input type=text name="cjjzprice" size=10 value=<%=ldjzunitprice%>  <%if (java.lang.Float.parseFloat(room_price)>0){ %>   readonly<%} %> onChange="document.cus.dicprice.value=subs1(parseFloat(this.value)*parseFloat(document.cus.jzarea.value),<%=sumlen %>);document.cus.cjtnprice.value=subs1(parseFloat(document.cus.dicprice.value)/parseFloat(document.cus.area.value),<%=unitlen %>);"></td>
      <td width="19%" class=roomleft >&nbsp;成交套内建<br>&nbsp;筑面积单价</td>
      <td width="19%" class=roomright >&nbsp;<input type=text name="cjtnprice" size=8  value=<%=ldunitprice%> <%if (java.lang.Float.parseFloat(room_price)>0){ %>   readonly<%} %> onChange="document.cus.dicprice.value=subs1(parseFloat(this.value)*parseFloat(document.cus.area.value),<%=sumlen %>);document.cus.cjjzprice.value=subs1(parseFloat(document.cus.dicprice.value)/parseFloat(document.cus.jzarea.value),<%=unitlen %>);"></td>  
     <td width="15%" class=roomleft>&nbsp;定 金</td>
      <td width="23%" class=roomright  >&nbsp;<input type=text name="earnest" value="<%=ldDJ%>" onblur="check(this);"   size="10"  >
        　</td>
	</tr>
	...
	<td width="19%" class=roomleft >&nbsp;合同计价方式</td>
	 ...
	 <!--
      ...
      <td width="19%" class=roomright >&nbsp;<input type=text name="wd_area_sumprice" size=8 readonly  value="<%=wdprice %>" > 元</td>  </tr>
	   ...
	   <tr><td colspan="4" class=roomleft>&nbsp;签约提醒延迟天数/日期:<input type="test" size=10 name="qydate"  ><input type=checkbox name=gddate value=1> 固定日期 </td>
	   </tr>
	   <script>
	   function countlutai(){
  ...
  <input type=hidden name=ajtxdate >
   <%if (Cortrol.indexOf("A")>=0) {%>
	...
	// if (checkdisc(document.cus.disc.value)){alert("折扣有误，可能是你没有此权限的折扣");document.cus.disc.focus();return false;}
      if (document.cus.qydate.value==""){alert("签约提醒日期不能为空");document.cus.qydate.focus();return false;}
...
openwin3("disc.jsp?code=<%=SecNo+Loft+RoomNo %>&discPrice="+document.cus.dicprice.value+"&discUnit="+document.cus.cjjzprice.value,700,400,100,100);
 ...
 var dddd="<%=payment%>";
 if (dddd.indexOf("揭")>=0)document.cus.ajmoney.disabled=false;
 ykj_count();
	 </script>
</form>
</body>
