...
<%@ include file="../public_js/Repalce.jsp"%>
 ...
 if (state.equals("1")){state=" checkstate=1 and";checkedStr="checked";checkedStr1="";}
   ...
   if (PubPra.indexOf("员")<0)PubPra="1";
	...
	}else {orderStr = " order by visadate ";}
	if(orderby.equals("0")){
...
<form action="contract_check_view.jsp" method="post" name="frm">
...
<table  width="100%">
 <tr bgcolor="#EAEAEA">
   <td>
	...
	<%} %>
    客户姓名:<input type=text name=kehuxingming size=5 value="<%//= kehuxingming%>" >
    时间段: 从<input type=input name=time1 size=8 readonly value="<%//= time1%>" ><input type=button name=cale value=".." onclick="fPopUpCalendarDlg(time1);return false;">到<input type=input name=time2 size=8 readonly value="<%//= time2%>" ><input type=button name=cale value=".." onclick="fPopUpCalendarDlg(time2);return false;">
   
   合同编号:<input type=text name=htno size=7>
...
<td   align="left">&nbsp;<a href="#" onclick="openwin1('showContract.jsp?secno=<%=secno %>&roomno=<%=Room_no %>&build=&loft=<%=loft %>&Floor=<%=Floor %>',700,500,100,100)"><%=Customer%></a>　</td>
...
<%if (checkstate){ %>
<td  align="left"><a href="#" onclick="cancelCheck('<%=startnum %>');">撤回</a></td>
