...
<script>
function submitC(){
  document.frm.action="RoomCodeShowDS.jsp";
  document.frm.submit();
...
function ylroom(){
    ...
    var newWin = window.open(url,"roomfrm","height=350,width=510,left=150,top=50,resizable=yes,scrollbars=no,toolbar=no,location=no,directories=no,status=no,menubar=no");
    newWin.focus() ;
}
...
<body oncontextmenu="return false"  topmargin="1" onload="setSelectMad(document.frm.condi,'<%= condi %>');setSelectMad(document.frm.aa,'<%=getbyte1(request.getParameter("aa"))%>');"">
 
<div align="center">


<form name=frm method="post" action="RoomCodeShowDS.jsp">
...
<p CLASS=FontColor><b><font size="3" ><%if (!SecName.equals(Loft)){out.print(SecName);} %><%=Loft+strBuilding %>销控表</font></b></p>
 <table width="90%" border="0" CELLSPACING=1 CELLPADDING=3>
	...
	<td bgcolor='<%=yl1 %>'  align="center"><%  if (Cortrol.indexOf("Y")>=0){  %><a href="#" onclick="ylroom();return false;" ><input type=button name=yl value="预  留"  ></a><%}else{ %>预留<%} %><img src='../images/day2.gif' border='0' ></td>
   ...
   <tr> 
    ...
    String sqlArr="select   a.state,a.dis, a.tddate,a.Room_code,a.saleState,a.Floor,a.Room,a.area,a.code,a.floorshow,a.floortype,a.floormess,b.showfloors,a.sumprice from roomstateview a,roomno b where a.seccode+a.loft+a.room_code= b.SectionNo+b.region+b.roomno and a.seccode=";
		   ...
		   while (rsArr.next()){
			...
			//  if (!getbyte(rsArr.getString("state")).equals("3"))
	...
	<%if (fsfc.equals("")){ %>
	 ...
	 <%}else if (state.equals("2")){salenum++;%>
...
%>
	...
	avgprice="";
	 %>
	<tr> 
     <td align="center"  class=FloorC  width="10%"><font color="#ffffff"><div id="A<%=String.valueOf(java.lang.Math.abs(i)) %>w1"><%=i%></div></font></td>
		<%
		float IntAvgSumPrice=0;;
		float FatAreaSum=0;
		for(int j=intBeginR;j<=intEndR;j++){
