...
function sendfirst(para){
  ...
  document.cus.dicprice.defaultValue="<%=youhuiprice %>";
   setSelectMad(document.cus.counttype,"<%=count_type %>");
     ...
     setSelectMad3(document.cus.pay,"<%=payment1%>");//+"期*"+disc1+"折" 
...
function tfo(){	
     openwin("TFRecordInput.jsp?code=<%=code %>",500,450,100,10);
  ...
  function RList(){
	...
	//if (document.cus.dis){
	 // if (checkdisc(document.cus.dis.value)){alert("折扣有误，可能是你没有此权限的折扣");document.cus.dis.focus();return false;}
    //}
	 if (isNaN(document.cus.qydate.value))
         document.cus.gddate.value="1";
	 if (document.cus.ewyh.value!=""){
	     if (parseInt(document.cus.ewyh.value)>0&& document.cus.youhui_date.value==""){
	    alert("请选择优惠的日期"); document.cus.youhui_date.focus();return false;
	   }
