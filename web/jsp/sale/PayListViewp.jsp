...
out.println(ShowPay("select id,num,paydate ,money as scale,state1,state2,state3,state4,state5  from PayDetail where payCode ="+code+" and  state1='1' and state2='0' and state3=0 order by num",out,Cortrol));

//输出付款金额按指金额，日期按指定日期计算的期数。
out.println(ShowPay("select id,num,convert(char(24),date,111) as paydate,money as scale,state1,state2,state3,state4,state5  from PayDetail where payCode ="+code+" and state1='1' and state2='1' order by num",out,Cortrol));

//输出付款金额按百比计算，日期按相对天数计算的期数。
out.println(ShowPay("select id,num,PayDate ,scale,state1,state2,state3,state4,state5 from PayDetail where PayCode ="+code+" and state1='0' and state2='0' order by num",out,Cortrol));

//输出付款金额按百比计算，日期按指定日期计算的期数。
out.println(ShowPay("select id,num,convert(char(24),date,111) as paydate, scale,state1,state2,state3,state4,state5  from PayDetail where payCode ="+code+" and  state1='0' and state2='1' order by num",out,Cortrol));

//输出付款金额按指金额，日期按相对天数计算的期数,且为楼款
out.println(ShowPay("select id,num,paydate ,money as scale,state1,state2,state3,state4,state5  from PayDetail where payCode ="+code+" and  state1='1' and state2='0' and state3=1 order by num",out,Cortrol));
