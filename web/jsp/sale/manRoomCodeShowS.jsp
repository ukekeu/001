...
function searcherdata(){
  ...
  frm.action='manRoomCodeShowS.jsp';
  else
  frm.action='manRoomCodeShowS1.jsp'; 
...
String housename = userRs.getString("loft") ;
	...
	</select>&nbsp;<%if (!Loft.equals("")){ %> <input type=button name="EXCELTO" value="导出到EXCEL" onclick="document.frm.action='ExcelTomanRoomCodeShowS.jsp';document.frm.submit();">&nbsp; <input type="button" name="Button" value="打印预览" onClick="document.frm.action='printmanRoomCodeShowS.jsp';document.frm.submit();"><%} %>
</td>
	 </tr>
	 <%} %>
</table> 
...
<p CLASS=FontColor align="center"><b><font size="3" ><%if (!SecName.equals(Loft)){out.print(SecName);} %><%=Loft+strBuilding %>销控表</font></b></p>
 <table width="98%" border="0"  align="center" CELLSPACING=1 CELLPADDING=3>
	...
	<td  bgcolor="<%=cz1 %>" align="center">租赁
	 </td>
	...
	<td bgcolor="<%=ld1 %>"  align="center"><div id=ly<%=aaa %>></div></td>
	 <td bgcolor='<%=cz1 %>'  align="center"><div id=re<%=aaa %>></div></td 
   <td   align="center"><div id=sum<%=aaa %>> </div>
	...
	<td bgcolor="<%=ld1 %>"  align="center"><div id=l1<%=aaa %>></div></td>
	 <td bgcolor='<%=cz1 %>'  align="center"> <div id=re1<%=aaa %>></div> 
	</td>
   <td   align="center"> 
	</td>
	</tr>
   ...
   if (ssr.next()){
	...
	String sqlArr="select    a.dis, a.tddate,a.Room_code,a.saleState,a.Floor,a.Room,a.area,a.code,a.sumprice,a.floorshow,a.floortype,a.floormess,b.showfloors from roomstateview a,roomno b  where   a.seccode+a.loft+a.room_code= b.SectionNo+b.region+b.roomno and a.seccode=";
	 ...
	 <table border="0" width="100%" CELLSPACING=1 CELLPADDING=0 >
	   ...
	   <tr>
	    <td  width="1%" bgcolor="#800000" align="center" background=""><img src="../images/1.gif" border="0"></td>
		...
		for(int j=intBeginR;j<=intEndR;j++){
		   ...
		   NO=AsiicZH(j,BeR);//调用asiic转换方法
			...
			%>
		      <td align="center" <%  if (Cortrol.indexOf("Y")>=0){ %> colspan="2"<%} %> class=listtitle <%if (intEndR>15){ %>  <%}else{ %>width="5%" <%} %> ><font color="#ffffff"><%=NO%></font><!--<input type=checkbox name="Check<%=j%>" value="<%=j%>" checked onclick="LokeRoomNo('<%=strSection%>','<%=Loft %>','<%=j %>',this);">--></td>
			 <%}
			 }else if (!Lock.contains(String.valueOf(j))){//
			 %>
			 <td align="center"  <%if (Cortrol.indexOf("Y")>=0){ %> colspan="2"<%} %> class=listtitle width="10%"><font color="#ffffff"><%=NO%></font></td>
			 <%} %>	
			 <%}else if (!Lock.contains(String.valueOf(j))){
			 
			 %>
			 <td align="center"  <%  if (Cortrol.indexOf("Y")>=0){ %> colspan="2"<%} %> class=listtitle width="10%"><font color="#ffffff"><%=NO%></font><!--<input type=checkbox name="Check<%=j%>" value="<%=j%>" checked onclick="LokeRoomNo('<%=strSection%>','<%=Loft %>','<%=j %>',this);">--></td>
	...
	%><td align="center" class=FloorC width="1%"><font color="#ffffff"><div id=A<%=String.valueOf(java.lang.Math.abs(i))+1 %>w1><%=i%></div></font></td>
		...
		%>
		<%if (Cortrol.indexOf("Y")>=0){ %>
		 <td align="center" width="1%"><%  if (Cortrol.indexOf("Y")>=0&&!state.equals("2")&&!state.equals("3")&&!state.equals("")){  %> <input type=checkbox name=yl ylloft="<%=Loft %>" value="<%=RoomNo %>"><%} %></td>
	    <%} %>	 <td align="center" <%if (intEndR>15){ %>  <%}else{ %>width="5%" <%} %>  <%=bg %> <%if (dis.equals("1")){ %>title="锁定日期:<%=tddate %>"<%} %>>
	<%if (fsfc.equals("")){ %>
	  <%  if (state.equals("1")&&dis.equals("0")){nosalenum++; %>
	  <%  if (Cortrol.indexOf("A")>=0){  %><a href="#<%=RoomNo %>" onclick="opennewwin('roomdata.jsp','?secname=<%=SecName %>&secno=<%=strSection%>&roomno=<%=no%>&build=<%=strBuilding%>&loft=<%=Loft %>&Floor=<%=i %>&state=1',750,530,120,5)"><%} %><%=RoomNo%><%  if (Cortrol.indexOf("A")>=0){  %></a><%} %>
	  <% }else if (state.equals("1")&&dis.equals("1")){  %>
	   <a href="#"> <%=RoomNo%>[锁定] </a>
	  <%}else if (state.equals("3")){ salenum++;%>
       <%=RoomNo%> 
	  <%}else if (state.equals("2")){rgnum++;%>
	   <a href="#<%=RoomNo %>"  <%  if (Cortrol.indexOf("E")>=0){  %> title="如果未打印认购书,可修改认购书" onclick="opennewwin('modiorder.jsp','?secname=<%=SecName %>&secno=<%=strSection %>&roomno=<%=no%>&build=<%=strBuilding%>&loft=<%=Loft %>&Floor=<%=i %>&show=false',750,500,100,10)"<%} %>><%=RoomNo%></a> 
	 ...
	 <%}else if (state.equals("6")){ %>
	  ...
	  <a href="#<%=RoomNo %>"  <%  if (Cortrol.indexOf("E")>=0){  %> onclick="opennewwin('roomdata1.jsp','?secname=<%=SecName %>&secno=<%=strSection %>&roomno=<%=no%>&build=<%=strBuilding%>&loft=<%=Loft %>&Floor=<%=i %>&state=5',750,530,120,5)"<%} %>><%=RoomNo%></a>
...
function ylroom(para,para1,para2){
 ...
 l1<%=aaa %>.innerHTML = "<%=FormtD.getFloat(Float.parseFloat(String.valueOf(ld))/(ld+nosalenum+yl+salenum+rgnum)*100,2) %>%";
   re<%=aaa %>.innerHTML = "<%=rentnum %>";
  re1<%=aaa %>.innerHTML = "<%=FormtD.getFloat(Float.parseFloat(String.valueOf(rentnum))/(ld+nosalenum+yl+salenum+rgnum)*100,2) %>%";
