...
<%!
  ...
  %> <%
   ...
   if (PubPra.indexOf("员")<0)PubPra="0";
 ...
 String sel=(String)request.getParameter("sel");
    ...
    if(sel==null) sel="0";	  	
     	sql="select Top "+pageSize+"  * from PrintReportContract where yj=1 and  Code not in (Select top "+prenum +" code from PrintReportContract where   yj=1 and  state=2   "+find+stdate+FindWhere +findStr +orderStr+")  and   state= 2 "+find+stdate+FindWhere +findStr +orderStr;
		sqlcount="select count(*) as num from PrintReportContract where  yj=1 and  state= 2   " +find+stdate+FindWhere + findStr ;
...
<%}else{ %>	
   合同编号:<input type=text name=htno size=10>
    ...
    <input type=button value="查询" onclick="document.forms[0].submit();" >
	 ...
	 &nbsp;&nbsp;&nbsp;&nbsp;<input type=button name=chcq value="同意办理产权" onclick="checkcq();">
	 <%} %>
  </td>
 </tr>
</table>
...
<td  align="center">备  注</td>
   ...
   //按FIELDS顺序一GET　VALUE
	 ...
	 area1=FormtD.getFloat3(rs.getDouble("jzarea"),2);
     ...
     ysumprice=FormtD.getFloat3(rs.getDouble("ysumprice"),2);
	 salerprice=FormtD.getFloat3(rs.getDouble("salerprice"),2);
...
<%if (!cq){ %><input name="aa" value="<%=startnum %>" type="checkbox"><%} %>
</td>
