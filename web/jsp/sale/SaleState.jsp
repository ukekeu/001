...
<tr>
    <td class="<%if(sel.equals("0")){out.print("menuselect3");}else{out.print("selemenu4");}%>" align="center" width=88 height=21>
	  <a href="#" onclick="document.frm.sel.value=0;findSub('SaleState.jsp');"><font color='<%if(sel.equals("0")){out.print("#FFFFFF");}else{out.print("#000000");}%>'>认购登记</font></a>
	</td>
	  <td class="<%if(sel.equals("1")){out.print("menuselect3");}else{out.print("selemenu4");}%>" align="center" width=88 height=21>
	  <a href="#" onclick="document.frm.sel.value=1;findSub('sk_process.jsp');"><font color='<%if(sel.equals("1")){out.print("#FFFFFF");}else{out.print("#000000");}%>'>付款进度</font></a>
	</td>
...
<script>
 ...
 <%  if (Cortrol.indexOf("Y")>=0){  %> <input type=checkbox name=yl value="<%=RoomNo %>"><%} %><%  if (Cortrol.indexOf("A")>=0){  %> <a href="#" onclick="opennewwin('roomdata.jsp','?secname=<%=SecName %>&secno=<%=strSection%>&roomno=<%=no%>&build=<%=strBuilding%>&loft=<%=Loft %>&Floor=<%=i %>&state=1',680,530,120,5)"><%} %><%=RoomNo.substring(RoomNo.indexOf("-")+1)%><%  if (Cortrol.indexOf("A")>=0){  %></a><%} %>
	 ...
	 <%}else if (state.equals("2")){rgnum++;%>
	    <a href="#"  <%if (Cortrol.indexOf("F")>=0){ %> title="如果未打印认购书,可修改认购书" onclick="opennewwin('modiorder.jsp','?secname=<%=SecName %>&secno=<%=strSection %>&roomno=<%=no%>&build=<%=strBuilding%>&loft=<%=Loft %>&Floor=<%=i %>&show=false',690,500,100,10)" <%}else{ %>title="无权打开"<%} %>> <%=RoomNo.substring(RoomNo.indexOf("-")+1)%> <%if (Cortrol.indexOf("F")>=0){ %></a><%} %> 
