...
String SQL="SELECT o.SerialNo,o.yong_tu,o.code,c.cus_name,c.phone,c.sj,c.id_card,c.address,o.sectionname,o.loft,o.room_no,";
   ...
   ResultSet rs=aBean.executeQuery(SQL);
	...
	rs.close();
    String upperMoney="";
    ResultSet execRs=aBean.executeQuery("exec dbo.L2U "+sump+",0 ");//大写金额转换存储过程
             if (execRs.next())
	...
	String upperMoney1="";
       ...
       execRs=aBean.executeQuery("exec dbo.L2U "+gzprice+",0 ");//大写金额转换存储过程
             if (execRs.next())
	...
	String upperMoney2="";
       ...
       execRs=aBean.executeQuery("exec dbo.L2U "+subscription+",0 ");//大写金额转换存储过程
             if (execRs.next())
...
<font size=3>
注意:在打印之前必须设置纸张，点击"打印预览"后再点击"打印"右的"图标"中选择,<br><br>选择纸张类型"A3",方向选择"横向"</font><!--<input class="buttoninput" type="button" value="打印预览" onclick="window.printdiv.style.display='none';factory.printing.Preview();"> -->
...
<TABLE style="LINE-HEIGHT: 180%" cellSpacing=0 cellPadding=0 width=614 
  border=0><TR valign="top"><TD width=630  align="right" ><%=SerialNo %></TD></TR>
</TABLE>
<TABLE style="LINE-HEIGHT: 180%" cellSpacing=0 cellPadding=0 width=614 
  border=0><TR><TD ></TD></TR>
</TABLE>
<TABLE style="LINE-HEIGHT: 180%" cellSpacing=0 cellPadding=0 width=614 
  border=0><TR><TD width=605  align="right" ><%//=SerialNo %></TD></TR>
</TABLE>
<TABLE style="LINE-HEIGHT: 180%" cellSpacing=0 cellPadding=0 width=614 
  border=0><TR><TD ></TD></TR>
</TABLE>
...
<TABLE style="LINE-HEIGHT: 180%" cellSpacing=0 cellPadding=0 width=614 
  border=0><TR><TD  width="80"></TD><TD width="250" >&nbsp;<%=cus_name %></TD><TD width="250" ><%=address %></TD></TR>
</TABLE>
<TABLE style="LINE-HEIGHT: 180%" cellSpacing=0 cellPadding=0 width=614 
  border=0><TR><TD  width="80"></TD><TD width="250" >&nbsp;<%=phone %>&nbsp;<%=sj %></TD><TD width="250" >&nbsp;<%=id_card %></TD></TR>
</TABLE>
<TABLE style="LINE-HEIGHT: 180%" cellSpacing=0 cellPadding=0 width=614 
...
<TABLE style="LINE-HEIGHT: 180%" cellSpacing=0 cellPadding=0 width=614 
  ...
  <TD  >&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;第<%=loft %>&nbsp;<%=room_no %>号房，用途为<%=yong_tu %>,目前暂测套内建筑面积约：<%=area %>M<sup>2</sup>，整套建筑面积为：<%=jzarea %>M<sup>2</sup>，分摊面积为：<%=ftarea %>M<sup>2</sup>，
   套内建筑面积单价： <%=tnunitprice %>元/M<sup>2</sup>，露台面积为：<%=lu_tai_area %>M<sup>2</sup>，露台单价：<%=lu_tai_area_price %>元/M<sup>2</sup>，花园面积为：<%=ty_area %>M<sup>2</sup>，花园单价：<%=ty_area_price %>元/M<sup>2</sup>，<%if (wd_area>0){ %>屋面面积：<%=wd_area %>M<sup>2</sup><%} %>(花园/露台/屋面面积不计产权面积),合计人民币:<%=upperMoney %>(小写￥<%=sump %>元)
  </TD</TR>
...
<TABLE style="LINE-HEIGHT: 180%" cellSpacing=0 cellPadding=0 width=614 
  border=0><TR valign="top"><TD width="390" align="right">></TD></TR><TR valign="top"><TD width="390" align="right"></TD></TR>
 <TR valign="top"><TD width="380" align="right"><%=upperMoney2 %><!--(￥<%=subscription %>元)--></TD></TR>
</TABLE>
 
<TABLE style="LINE-HEIGHT: 180%" cellSpacing=0 cellPadding=0 width=614 
  border=0><TR valign="top"><TD width="390" align="right"><%//=upperMoney2 %><!--(￥<%=subscription %>元)--></TD></TR>
</TABLE>
<TABLE style="LINE-HEIGHT: 180%" cellSpacing=0 cellPadding=0 width=614 
...
<TABLE style="LINE-HEIGHT: 180%" cellSpacing=0 cellPadding=0 width=654 
  border=0>
  <%
  ...
  String kx="";
   ...
   %>
  <TR><TD width="100" >&nbsp;</TD><TD ><%=dd %>交款时间：<%=rs.getDate("shouldDate").toString() %>，款项名称:<%=kx %>，交款金额:￥<%=Format.getFloat3(rs.getDouble("shouldMoney"),2) %>元<%if (!kx.equals("花园/露台款")){ %>,占总房款的<%=Format.getFloat3(rs.getDouble("aa"),2) %>%<%} %>；
  ...
  if (payment1.indexOf("按揭")>=0){
   %>
   <TR><TD width="100" >&nbsp;</TD><TD >其余的款项目金额￥<%=ajmoney %>，占总房款的<%=ajrate %>%，由银行提供<%=ajyear %>年按揭。
   </TD></TR>
   <%} %>
</TABLE>
<TABLE style="LINE-HEIGHT: 180%" cellSpacing=0 cellPadding=0 width=614 
