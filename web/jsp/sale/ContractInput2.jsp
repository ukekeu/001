...
String floors=request.getParameter("Floor");
  ...
  if(see==null)see="";
     sqlrg="select * from fsfc where roomcode="+request.getParameter("roomcode");
	  ResultSet rs3=ViewBean.executeQuery(sqlrg);
	  if (rs3.next()){
  //读取合同资料
      sqlrg="select  o.yjrate,o.cqfe,o.yjfe,o.gjj,o.gjjy,o.sydk,o.sectionname,o.floors,o.loft,o.section,o.jl_bak,o.jldate, o.countType,o.gxr,o.code,o.serialno,o.ContractNo,o.Customer,o.sectionname as secname,o.building,";
	 ...
	 sqlrg+=" o.rmbprice-o.zs_first_money as factprice ,(o.rmbprice-o.zs_first_money)/o.jzarea as fsalejzPrice,";
	  ...
	  sqlrg+=",ty_area,ty_area_price,ty_area_price*ty_area typrice,y_area,y_jzarea,o.wd_area,o.wd_area_price,o.wd_area_price*o.wd_area wdprice,memberno,membert from  Order_Contract o,customer c,fsfc d   ";
	  sqlrg+=" where o.state<>3 and   o.Customer=c.serialNo and ";
	  sqlrg+="o.code=d.order_contract and d.roomcode="+request.getParameter("roomcode");
     }else{
	  sqlrg="select  o.yjrate,o.cqfe,o.yjfe,o.gjj,o.gjjy,o.sydk,o.sectionname,o.floors,o.loft,o.section,o.jl_bak,o.jldate, o.countType,o.gxr,o.code,o.serialno,o.ContractNo,o.Customer,o.sectionname as secname,o.building,";
	  sqlrg+="o.Room_no,o.buildtype,o.roomtype,o.payment,o.payment1,(salerprice-zs_first_money)/o.jzarea as sjsalejzPrice,(salerprice-zs_first_money)/o.area as sjsaleUsePrice,";
	  sqlrg+="o.area,o.zx,o.unitprice,o.tnunitprice,o.salerprice,o.unitprice as salejzPrice,o.tnunitprice saleUsePrice,o.AjMoney,o.MonthPayMoney,";
	 ...
	 rs3.close();
	    rs3=ViewBean.executeQuery(sqlrg);
	   String gzstate="",bxstate="",jldate="";
	   String code="",gxr="",jlbak="";
    
	   String rgno="",yh_zs="",cqfe="100",yjfe="100";
	   String youhui_date="";
	   String ContractNo="";
	   String secname="";
	   String buildtype="";
	 ...
	 String wd_area="0",wd_area_price="0",wdprice="0",yjrate="0";
	  while(rs3.next()){  
	    sectionname=getbyte(rs3.getString("sectionname"));
	    floors=getbyte(rs3.getString("floors"));
		Floor=floors;
		yjrate=getbyte(rs3.getString("yjrate"));
	    secno=getbyte(rs3.getString("section"));
		SecNo=secno;
		loft=getbyte(rs3.getString("loft"));
		Loft=loft;
		cqfe=getbyte(rs3.getString("cqfe"));  
	    yjfe=getbyte(rs3.getString("yjfe"));
	  ...
	  rs3.close(); 
	   ResultSet wRs=ViewBean.executeQuery("select state_type from hsm_ajk where section_no='"+secno+"'");
   if (wRs.next()){
...
String RoomSql="select floor,Code,case when gl_unitprice>0 then  gl_area+UsedArea else UsedArea end UsedArea,case when gl_unitprice>0 then  gl_area+Area else Area end    Area,Unitprice,TNunitprice,gl_area*gl_unitprice+Sumprice Sumprice,Fitment,RoomType,Room_Type,floor ";
...
<input type=hidden name=hisumprice value="<%=salprice%>">
  <p align=center><font size=3><b>销售资料登记</b></font></p>
   ...
   <tr> 
	  ...
	  <input type=button name=choose1  onclick="tfo();" value="退 定"> 
	  <input type=button name=choose2  onclick="RList();"value="换 房">
...
<tr> 
 ...
 <table BORDER=0 width="100%" CELLSPACING=0 CELLPADDING=0 ALIGN=center bordercolor="#ffffff" class="tablefill">
      ...
      <td width="19%" align=left class=roomright><div id="idname"><%=sectionname%></div> 
        <input type=hidden name="sectio" readonly value="<%=sectionname%>" size="10"  >
         <input type=hidden name="oldsectio" readonly value="<%=sectionname %>" size="10"  >
       
	    <input type=hidden name="section"  value="<%=secno %>" size="10"  >
           <input type=hidden name="oldsection"  value="<%=secno%>" size="10"  >
   
	  </td>
      <td width="15%" align=left class=roomleft>&nbsp;房&nbsp;间&nbsp;号&nbsp;</td>
      <td   align=left class=roomright width="20%"><div id="idroomno"><%=loft %><%=getbyte2(request.getParameter("build")) %><%=RoomNo%></div></td>
      <td   align=left class=roomleft width="12%">&nbsp;交楼日期</td>
      <td width="19%" class=roomright align=left>&nbsp;<%=JLdate.substring(0,10) %> 
        　</td>
    </tr>
	 
	    <input type=hidden name="roomno" readonly value="<%=RoomNo %>" size="15"   onChange="toUpper(document.cus.roomno)">
        <input type="hidden" name="loft" readonly="readonly" size="15" value="<%=loft%>">
		<input type="hidden" name="oldloft" readonly="readonly" size="15" value="<%=loft%>">
 ...
 function bb(){
   if (document.cus.editnow.value=='2'){
    ...
    <tr> 
      ...
      <td class=roomright>&nbsp; <input type=text  name="carid"  size=50  value="<%=ID_Card %>"  onchange="countAge();" onkeydown="key(document.cus.tel)">
        　</td>
    </tr><input type=hidden name=age>
    <tr> 
      <td width="16%" class=roomleft>&nbsp;联系电话</td>
      <td class=roomright >&nbsp; <input type=text  name="tel"  size=30   value="<%=Phone %>" onKeyDown="key(document.cus.bbj)">
        手机： 
        <input type=text  name="sj"  size=21  value="<%=sj %>" onKeyDown="key(document.cus.addr)">
        　</td>
    </tr>
    <tr> 
      <td width="16%" class=roomleft>&nbsp;联系地址</td>
      <td class=roomright >&nbsp; <input type=text  name="Address"  size=60  value="<%=Address %>" onKeyDown="key(document.cus.shouji)"> 
      </td>
    </tr>
     <tr> 
      <td width="16%" class=roomleft>&nbsp;产权占有份额</td>
      <td class=roomright >&nbsp; <input type=text  name="cq"  size=60   value="<%=cqfe %>" title="如果是多业主，请在产权占有份额之间用/分隔，且与业主名称一一对应" onKeyDown="key(document.cus.memberT)"> 
      </td>
    </tr>
	<tr> 
      <td class=roomleft>&nbsp;是否业主介绍： </td>
      <td class=roomright>&nbsp; <input type=checkbox name=memberT value=1  <%if (membert){out.print("checked");} %>  ><input type=text name=memberNO  readonly value="<%=memberno %>" maxlength="50"><input type=button name=FindC  value="查找" onclick="FindOption1();"> 
	  </td>
	  </tr>
    <tr> 
   ...
   <option value=1>需要入户</option>
        ...
        <input type=hidden name=demurrage value=0>佣金提成份额<input type=text name=yjfe  value="<%=yjfe %>" size=13 title="如果是两个销售员，请在占有份额之间用/分隔"></tr>
    <tr> 
     <td width="16%" rowspan="5"   class=roomleft>&nbsp;备&nbsp;注</td>
      <td  class=roomright>&nbsp; <textarea rows="3" name="note" cols="25"  ><%=note %></textarea> &nbsp;销售员:&nbsp; <select name="seller"  multiple size=3  onblur="SelectedSeller();" style="width:100">
        ...
        <input type=button name=rem value=">>" onclick="remo();"  title="删除销售员">&nbsp;提成比例: <input type=text name="yjrate"  value="<%=yjrate %>" size="3" onBlur="checkedOrderNoExist();">‰ 
...
<input type="hidden" name=oldajcs size=3 value=<%=oldajcs %>>
   ...
   %>
		...
		<td class=roomright>&nbsp;<input type=text name=gjj size=10 value="<%=gjj %>" OnBlur="dkcount1();">(元)</td>
		 <td width="12%" class=roomleft >&nbsp;按揭年限</td> 
   ...
   <td class=roomleft>&nbsp;商业  贷款</td>
		<td class=roomright>&nbsp;<input type=text name=sydk size=10  value="<%=sydk %>" OnBlur="dkcount2();">(元)</td>
...
<input type="hidden" name=ajcs size=3 value=<%=ajcs %>>
	...
	<input type=hidden name=yukuan value=0>
      <input type=hidden name=oldprice value="<%=room_price %>">
...
<td width="19%" class=roomleft >&nbsp;成交房款</td>
   ...
   <input type=hidden name="countdiscprice" value="<%=room_price %>">
      ...
      <td width="19%" class=roomright  >&nbsp;<input  type=text size=10  value="<%=firstmoney%>" name=firstMoney   onblur="check(this);countsq();">(元)　</td>
  ...
  <input type=hidden name="Adicprice"     size="10" value="<%=salprice %>" onkeydown="key(document.cus.cjjzprice);"> 
    ...
    <tr>
      ...
      <td width="23%" class=roomright  >&nbsp;<input type=text value="<%=dj %>" name="earnest"  onblur="check(this);"   size="10"  >
    </td>     
    </tr>
...
<tr> 
 ...
 double s_price=0;
    ...
    <tr bordercolor="#FFFFFF"> 
	 ...
	 <tr><td colspan="4" class=roomleft>&nbsp;签约后<input type="test" size=10 value=7 name="batxdate"  >天之内备案</td>
	   </tr>
	 <tr bordercolor="#FFFFFF"> 
 
     
      <td width="18%"  colspan="6" class=roomright>&nbsp;重新计算付款计划:<input type=checkbox name=modijh value=1>&nbsp;重新计算税费:<input type=checkbox name=moditax value=1>&nbsp;&nbsp;<font color=red>当客户的付款方式或总房款变化时，建议选择本选择</font></td>
    </tr>
