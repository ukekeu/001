...
　　　　return Math.ceil(rnd()*number); 
 ...
 <jsp:useBean id="ViewBean" scope="page" class="ConnDatabase.SDatabase"/>
  ...
  <jsp:useBean id="FormtD3" scope="page" class="test.BarChart"/>  
...
<table border="1" cellpadding="0" cellspacing="0" style="border-collapse: collapse" bordercolor="#111111" width="88%" id="AutoNumber1">
  ...
  if (!strLoft.equals(""))
    SQL="select sum(AREA) area,sum(AREA*unitprice) sumprice,count(*) num from room  where not seccode+loft+room_code in (select section+loft+room_no from order_contract  where state<>3 and section='"+strSecNo+"'  and loft='"+strLoft+"' ) and  seccode='"+strSecNo+"'  and loft='"+strLoft+"'";
        ...
        SQL="select sum(AREA) area,sum(AREA*unitprice) sumprice,count(*) num from room  where not seccode+loft+room_code in (select section+loft+room_no from order_contract  where state<>3 and section='"+strSecNo+"'  ) and  seccode='"+strSecNo+"'";
...
　</center>
<a href="#" onclick="openwin('saleFx.jsp',500,450,100,100);">按图形分析</a>&nbsp;&nbsp;<!--<a href="#" onclick="openwin('autoselect10.jsp',300,400,100,100);">按时段对比统计</a>-->&nbsp;&nbsp;<a href="#" onclick="document.frm.action='DataCount.jsp';document.frm.submit();">按销售均价对比分析</a>
...
%>
     <%
	  SQL="SELECT  sum(jzarea) jzarea,convert(char(7),date,120)  as yearmonth FROM order_contract  where state<>3 and  section='"+strSecNo+"'   and year(date)="+String.valueOf(nowyear)+" group by convert(char(7),date,120)";
     if (!strLoft.equals(""))
	  SQL="SELECT   sum(jzarea) jzarea,convert(char(7),date,120)  as yearmonth FROM order_contract  where state<>3 and  section='"+strSecNo+"' and loft='"+strLoft+"' and year(date)="+String.valueOf(nowyear)+" group by convert(char(7),date,120)";
     String   SQL1="SELECT   SUM(AREA)/10 jzarea from room where seccode='"+strSecNo+"'";
     if (!strLoft.equals(""))
     SQL1="SELECT  SUM(AREA)  jzarea from room where seccode='"+strSecNo+"' and loft='"+strLoft+"'";
   
   
  String SQL2="SELECT   sum(area)/10 jzarea FROM room  where    seccode+loft+room_code in (select section+loft+room_no from order_contract  where state<>3  and section='"+strSecNo+"'  and year(date)<="+String.valueOf(nowyear)+") and  seccode='"+strSecNo+"'";
  if (!strLoft.equals(""))
   
 SQL2="SELECT   sum(area) jzarea FROM room  where   seccode+loft+room_code in (select section+loft+room_no from order_contract  where state<>3 and section='"+strSecNo+"'  and loft='"+strLoft+"' and year(date)<="+String.valueOf(nowyear)+") and  seccode='"+strSecNo+"'  and loft='"+strLoft+"'";

  String SQL3="SELECT   sum(area)/10 jzarea FROM room  where not  seccode+loft+room_code in (select section+loft+room_no from order_contract  where state<>3  and section='"+strSecNo+"'  and year(date)<"+String.valueOf(nowyear)+") and  seccode='"+strSecNo+"'";
  if (!strLoft.equals(""))
   
   SQL3="SELECT   sum(area) jzarea FROM room  where not seccode+loft+room_code in (select section+loft+room_no from order_contract  where state<>3 and section='"+strSecNo+"'  and loft='"+strLoft+"' and year(date)<"+String.valueOf(nowyear)+") and  seccode='"+strSecNo+"'  and loft='"+strLoft+"'";
 
 String SQL4="SELECT   sum(jzarea)/10 jzarea FROM order_contract  where state<>3 and  section='"+strSecNo+"'  and year(date)="+String.valueOf(nowyear);
  if (!strLoft.equals(""))
  SQL4="SELECT  sum(jzarea) jzarea FROM order_contract  where state<>3 and  section='"+strSecNo+"' and loft='"+strLoft+"' and year(date)="+String.valueOf(nowyear);
 
    String picurl=strSecNo;
  
	Calendar calendar = Calendar.getInstance();
	String filename = String.valueOf(calendar.getTimeInMillis());	
	photo= FormtD3.main1("D:/"+ request.getContextPath()+"/crmv9/jsp/picture/pic3"+filename+".jpg",SQL,"月销售面积情况统计图","面积(平方米)","年月",SQL1,SQL2,SQL3,SQL4); %>
