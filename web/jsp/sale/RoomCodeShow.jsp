...
Cortrol+=checke_qx(InsertBean,(String)session.getAttribute("loginid"),"S002");
 ...
 <table width="90%" border="0" CELLSPACING=1 CELLPADDING=3>
   <tr><td bgcolor='<%=ws1 %>' align="center">未售</td>
	...
	<td bgcolor='<%=yl1 %>'  align="center"><%  if (Cortrol.indexOf("Y")>=0){  %><a href="#" onclick="ylroom(1);return false;" >预留</a><%}else{ %>预留<%} %><img src='../images/day2.gif' border='0' ></td>
	
	<td bgcolor="<%=ld1 %>"  align="center">临定<img src='../images/face39.gif' border='0' ></td>
	<td   align="center"><div id=sum><div id=pri><input type=button name=print value="打 印" title="打印销控表" onclick="printT()">&nbsp;<%  if (Cortrol.indexOf("Y")>=0){  %><input type=button name=print value="锁定" title="锁定单元" onclick="Aloke()">&nbsp;<input type=button name=print value="解锁" title="解除锁定单元" onclick="Cloke()"><%} %></div></td>
   ...
   <td   align="center"><div id=sum><select name=aa onchange="px(this.value);">
	...
	<td bgcolor="<%=ld1 %>"  align="center"><div id=l1></div></td>
	 <td   align="center"><input type=checkbox name=roomcx value=1 <%if (getbyte1(request.getParameter("roomcx")).equals("1")){out.print("checked");} %> onclick="document.frm.submit();">&nbsp;朝向&nbsp;<input type=checkbox name=roomdj value=1 <%if (getbyte1(request.getParameter("roomdj")).equals("1")){out.print("checked");} %> onclick="document.frm.submit();">&nbsp;预售单价&nbsp; 
	</td>
   <td   align="center">
	</td>
	</tr>
...
<script>
  ...
  <table width="90%" border="0" CELLSPACING=1 CELLPADDING=3>
   <tr> 
		<td width="10%"  bgcolor="#800000" align="center"><font color="#FFFFFF"<b><%if (!Rinfo.equals("")) {out.print(Rinfo+"单元");}%></b></font></td>
		<%
...
String sqlArr="select   a.state,a.dis, a.tddate,a.Room_code,a.saleState,a.Floor,a.Room,a.area1 area ,a.code,a.sumprice,a.floorshow,a.floortype,a.floormess,b.showfloors,a.roomtype,a.Direction,a.unitprice from roomstateview a,roomno b where a.seccode+a.loft+a.room_code= b.SectionNo+b.region+b.roomno and a.seccode=";
     ...
     <td align="center"  class=FloorC width="10%"><font color="#ffffff"><div id=w1<%=String.valueOf(java.lang.Math.abs(i)) %>><%=i%></div</font></td>
		...
		// if (state.equals("6")){ bg="bgcolor='"+ws1+"'";}
		 if (state.equals("7")){ bg="bgcolor='"+cz1+"'";rentnum++;}
		  no=RoomNo;
		 if (condi.equals("wen")){bg="bgcolor='#c0c0c0'";
		...
		%> 
	  <td align="center"   <%=bg %>  width="10%" <%if (dis.equals("1")){ %>title="锁定日期:<%=tddate %>"<%} %>>
	 <%if (fsfc.equals("")){ %>
	...
	<%  if (Cortrol.indexOf("A")>=0){  %> <input type=checkbox name=yl value="<%=RoomNo %>"><a href="#<%=RoomNo %>" onclick="opennewwin('roomdata.jsp','?secname=<%=SecName %>&secno=<%=strSection%>&roomno=<%=no%>&build=<%=strBuilding%>&loft=<%=Loft %>&Floor=<%=i %>&state=1',720,530,120,5)"><%} %><%=RoomNo%><%  if (Cortrol.indexOf("A")>=0){  %></a><%} %>
	 ...
	 <%}else if (state.equals("2")){rgnum++;%>
	  <a href="#<%=RoomNo %>" title="如果未打印认购书,可修改认购书" onclick="opennewwin('modiorder.jsp','?secname=<%=SecName %>&secno=<%=strSection %>&roomno=<%=no%>&build=<%=strBuilding%>&loft=<%=Loft %>&Floor=<%=i %>&show=false',750,500,100,10)"><%=RoomNo%> </a> 
	...
	<%  if (Cortrol.indexOf("Y")>=0){  %>   <input type=checkbox name=yl value="<%=RoomNo %>"><a href="#<%=RoomNo %>" onclick="choose('<%=RoomNo%>');ylroom(2);"><%} %><%=RoomNo%> <%=l5 %><%  if (Cortrol.indexOf("A")>=0){  %></a><%} %>
	 ...
	 <%}else{ %>&nbsp;<%} %>
	 <%}else if (state.equals("1")&&!dis.equals("1")){ %>
	    <a href="#<%=RoomNo %>" onclick="opennewwin('roomdata.jsp','?secname=<%=SecName %>&secno=<%=strSection %>&roomno=<%=no%>&build=<%=strBuilding%>&loft=<%=Loft %>&Floor=<%=i %>&state=5',700,530,120,5)"><%=RoomNo%>  </a>
	
	
	 <%}else  if (state.equals("2")){%>
	  <a href="#<%=RoomNo%>" onclick="opennewwin('ContractInput2.jsp','?roomcode=<%=l6 %>&secname=<%=SecName %>&secno=<%=strSection %>&roomno=<%=no%>&build=<%=strBuilding%>&loft=<%=Loft %>&Floor=<%=i %>&show=false',680,530,120,2);"><%=RoomNo%> </a> 
	
	 <%} %><%if (getbyte1(request.getParameter("roomarea")).equals("1")){ %><%=l5 %><%} %> <%if (getbyte1(request.getParameter("roomfx")).equals("1")){ %><%=showRoomtype %><%} %>  <%if (getbyte1(request.getParameter("roomcx")).equals("1")){ %><%=showDirection %><%} %><%if (getbyte1(request.getParameter("roomdj")).equals("1")){ %><%=showUnitprice %><%} %></td> 
		<%}%>
...
%>
	...
	<%  if (Cortrol.indexOf("A")>=0){  %> <input type=checkbox name=yl value="<%=RoomNo %>"><a href="#<%=RoomNo %>" onclick="opennewwin('roomdata.jsp','?secname=<%=SecName %>&secno=<%=strSection%>&roomno=<%=no%>&build=<%=strBuilding%>&loft=<%=Loft %>&Floor=<%=i %>&state=1',680,530,120,5)"><%} %><%=RoomNo%> <%  if (Cortrol.indexOf("A")>=0){  %></a><%} %>
	 ...
	 <%}else if (state.equals("3")){salenum++;%>
	  <%=RoomNo%> 
	 <%}else if (state.equals("4")){yl++;%>
	<%  if (Cortrol.indexOf("Y")>=0){  %>   <input type=checkbox name=yl value="<%=RoomNo %>"><a href="#<%=RoomNo %>" onclick="choose('<%=RoomNo%>');ylroom(2);"><%} %><%=RoomNo%><%  if (Cortrol.indexOf("A")>=0){  %></a><%} %>
	 <%}else if (state.equals("6")){ %>
	  <a href="#<%=RoomNo %>" onclick="opennewwin('ContractList.jsp','?secname=<%=SecName %>&secno=<%=strSection %>&roomno=<%=no%>&build=<%=strBuilding%>&loft=<%=Loft %>&Floor=<%=i %>&show=false',480,500,150,10)"><%=RoomNo%></a> 
	  <%}else if (state.equals("5")){ ld++;%>
