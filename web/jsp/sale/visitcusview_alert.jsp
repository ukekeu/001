...
<%  
     ...
     %> </select>
	 &nbsp;&nbsp;<select name="order" onchange="changesub(this.value);"><option  value="">按跟踪日期排序</option><option value="visit_date">按来访日期排序</option><option value="cus_name">按来访姓名排序</option></select>
...
　
 ...
 sql="select b.*,convert(char(10),a.date,120) date,a.num  from tail_after a,customer b where a.state=0 and a.cusno=b.serialno and cusno in (select cusno from      tail_after group by  cusno  ) and dateadd(dd,alert_day,a.date)<getdate() "+lffs +" and b.Section in("+SectionS+")  and b.SerialNo not in (select customer from order_contract where state<>3 and section in("+SectionS+")) "+strWhere;
...
int tr=0;
   ...
   while (rs.next()){
	 ...
	 String gz_date=getbyte(rs.getString("date"));
 ...
 <td align="center"><%=region %>&nbsp;</td>
    <td  align="center"><a href="#" onclick="openwin('../customter/editcustomer.jsp?cusno=<%=no %>',560,560,100,5);" title="点击可以弹出窗口修改资料"><%=name %></a>　&nbsp;</td>
     <td  align="center">
	<%String strt="";
	String strs="";
...
<script>
   ...
   var cusno="";
    function Edit(para){
		     ...
		     else{ openwin("../customter/CusFeedBack.jsp?cusno="+para,560,560,100,5);}
