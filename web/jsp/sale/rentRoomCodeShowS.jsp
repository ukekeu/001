...
<form name=frm method="post" action="rentRoomCodeShowS.jsp">
 ...
 String ss="select room_type,ts from LoftBuildCode where SECNO='"+strSection+"' and loft='"+Loft+"'";
   ...
   if (ssr.next()){
	   ...
	   <tr>
		...
		for(int j=intBeginR;j<=intEndR;j++){
		   ...
		   NO=AsiicZH(j,BeR);//调用asiic转换方法
			...
			%>
		      <td align="center"   class=listtitle <%if (intEndR>15){ %>  <%}else{ %>width="5%" <%} %> ><font color="#ffffff"><%=NO%></font><!--<input type=checkbox name="Check<%=j%>" value="<%=j%>" checked onclick="LokeRoomNo('<%=strSection%>','<%=Loft %>','<%=j %>',this);">--></td>
			 <%}
	...
	<%if (fsfc.equals("")){ %>
	 ...
	 <a href="#<%=RoomNo %>"  <%  if (Cortrol.indexOf("E")>=0){  %> onclick="opennewwin('zl_list.jsp','?secname=<%=SecName %>&secno=<%=strSection %>&roomno=<%=no%>&build=<%=strBuilding%>&loft=<%=Loft %>&Floor=<%=i %>&state=5',750,530,120,5)"<%} %>> <%=RoomNo%></a>
	 <%}else{ %>&nbsp;<%} %>
...
%>
	...
	<%  if (Cortrol.indexOf("A")>=0){  %><a href="#<%=RoomNo%>" onclick="opennewwin('roomdata.jsp','?secname=<%=SecName %>&secno=<%=strSection%>&roomno=<%=no%>&build=<%=strBuilding%>&loft=<%=Loft %>&Floor=<%=i %>&state=1',720,530,120,5)"><%} %><%=RoomNo%><%  if (Cortrol.indexOf("A")>=0){  %></a><%} %>
	 ...
	 <%=RoomNo%>  
	  ...
	  <%=RoomNo%>
	 <%} else if (state.equals("7") ){%>
	 <a href="#<%=RoomNo%>" onclick="opennewwin('zl_list.jsp','?secname=<%=SecName %>&secno=<%=strSection %>&roomno=<%=no%>&build=<%=strBuilding%>&loft=<%=Loft %>&Floor=<%=i %>&state=5',680,530,120,5)"> <%=RoomNo%></a>
	 <%}else{ %>&nbsp;<%} %>
	 </td>
