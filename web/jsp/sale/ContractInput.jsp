...
<form method="POST" name="cus" action="ContractInput.jsp"> 
...
<tr  valign="top">
    <td class="<%if(sel.equals("1")){out.print("menuselect3");}else{out.print("selemenu4");}%>" align="center" width=94 height=21>
	  <a href="#" onclick="document.cus.sel.value=1;findSub('ContractInput.jsp');"><font color='<%if(sel.equals("1")){out.print("#FFFFFF");}else{out.print("#000000");}%>'>合同资料</font></a>
	</td>
	<td class="<%if(sel.equals("2")){out.print("menuselect3");}else{out.print("selemenu4");}%>" align="center" width=94 height=21>
	 <a href="#" onclick="document.cus.sel.value=2;findSub('ContractInput.jsp');"><font color='<%if(sel.equals("2")){out.print("#FFFFFF");}else{out.print("#000000");}%>'>付款情况</font></a>
	</td>
	<td class="<%if(sel.equals("3")){out.print("menuselect3");}else{out.print("selemenu4");}%>" align="center" width=94 height=21>
	 <a href="#" onclick="document.cus.sel.value=3;findSub('ContractInput.jsp');"><font color='<%if(sel.equals("3")){out.print("#FFFFFF");}else{out.print("#000000");}%>'>办证情况</font></a>
	</td>
	 <script>
...
<tr> 
     ...
     <input type=hidden name="cusname2" value="<%=cusname %>">
      <td colspan="5" class=roomright>&nbsp;<input type=text   name="cusname1"  size=60 onchange="aa();" value="<%=cusname %>"  onkeydown="key(document.cus.carid)" title="如果多个姓名，请用'/'分隔"><input type=hidden name="editnow"  　 value="1">       　 
 ...
 function bb(){
   if (document.cus.editnow.value=='2'){
    ...
    <tr> 
      ...
      <td class=roomright colspan="5">&nbsp;<input type=text name=youhui onKeyDown="key(document.cus.custype)" size=50 value="<%=yh_zs %>"> 
   ...
   <option value=1>需要入户</option>
    ...
    <tr> 
     <td width="16%"    class=roomleft>&nbsp;备&nbsp;注</td>
      <td  class=roomright colspan="5">&nbsp;<textarea rows="3" name="note" onKeyDown="key(document.cus.listlar)" cols="40"  ><%=note %></textarea> &nbsp;销售员&nbsp; <select name="seller"  multiple size=3  onblur="SelectedSeller();" style="width:100">
...
<td width="19%" class=roomleft >&nbsp;成交房款</td>
  ...
  <input type=hidden name="Adicprice"     size="10" value="<%=salprice %>" onkeydown="key(document.cus.cjjzprice);"> 
    ...
    <tr>
      ...
      <td width="23%" class=roomright  >&nbsp;<input type=text value="<%=dj %>" name="earnest"  onblur="check(this);"   size="10" title="如果填写0，表示按付款方式公式为准" > 
    </td>     
    </tr>
...
<tr> 
    ...
    <tr > 
      ...
      <td width="18%"  class=roomright>&nbsp;<input type=text size=10 value="<%=factmoney %>"><input type=hidden name="firstFT" value="1" style="width : 105"></td>
      ...
      <td width="18%"  class=roomright>&nbsp;<INPUT TYPE="text"  NAME=QYDate value="<%if (state.equals("2"))out.println(VisaDate.substring(0,10)); %>" size=10   title="请选择签约日期"   onkeydown="key(document.cus.kkrate)"><input type=button name=cale value=".." onclick="fPopUpCalendarDlg(QYDate);" title="请选择签约日期"></td>
    </tr>
	   <tr > 
 ...
 <input type="hidden" name="gddate" >
    <tr bordercolor="#FFFFFF"> 
      ...
      <td width="18%" class=roomright>&nbsp;<INPUT TYPE="text" value="<%if (!StrAjDate.equals("")&&Integer.parseInt(StrAjDate.substring(0,4))>=2000)out.println(StrAjDate.substring(0,10)); %>"  NAME=ajDate size=10  readonly></td>
      <td width="16%" class=roomleft>&nbsp;放款日期</td>
      <td width="18%" class=roomright>&nbsp;<INPUT TYPE="text" value="<%if (!StrFkDate.equals("")&&Integer.parseInt(StrFkDate.substring(0,4))>=2000)out.println(StrFkDate.substring(0,10)); %>"  NAME=fkDate size=10  readonly ></td>
    </tr>
    <tr bordercolor="#FFFFFF"> 
      <td width="16%" class=roomleft>&nbsp;公证状态</td>
      <td width="18%" class=roomright>&nbsp;<select name="gz" style="width : 80"><option value="0">未办</option><option value="1">已办</option></select></td>
      <td width="14%" class=roomleft>&nbsp;保险状态</td>
      <td width="18%" class=roomright>&nbsp;<select name="bx" style="width : 80"><option value="0">未办</option><option value="1">已办</option></select></td>
      <td width="16%" class=roomleft>&nbsp;保险金额</td>
      <td width="18%" class=roomright>&nbsp;<input type=text size=10 value="<%=StrBxMoney %>" name=bxmoney></td>
    </tr>
   <%
 ...
 double s_price=0;
    ...
    <tr bordercolor="#FFFFFF"> 
      ...
      <td width="18%" class=roomright>&nbsp;<input type=text size=10 name=tax value="<%=taxprice %>" readonly></td>
       <td width="16%" class=roomleft>&nbsp;配套合计</td>
      <td width="18%" class=roomright>&nbsp;<input type=text size=10 name=tax value="<%=peitaoprice %>" readonly></td>
     
	  <td width="14%" class=roomleft>&nbsp;首期合计</td>
      <td width="18%" class=roomright>&nbsp;<input type=text size=10 name=tax value="<%=Format.getFloat3(s_price,2) %>" readonly></td>
    ...
    <input type=hidden name=ajtxdate value="0">
	 <input type=hidden name=orderstate value="1">
	  <input type=hidden name=G_jzarea value="0">
  </table>
...
<input type=button name=add value="补收项目" onclick="addtax();">
 <table width="100%"  border="0" cellspacing="1"  cellpadding="1"  >
    ...
    <%
String sql1="";
  String 		sql="select taxname_type,id,TaxName,TaxMoney,state,TaxMoney-factmoney q from GatheringTaxRecord where contactno ='"+code+"'";//order by room_no desc";
//设置查询结果的存放变量
int ID=0;
String CusNo="";
String CusName="";
  ...
  //按FIELDS顺序一GET　VALUE
   ...
   TaxMoney=rs.getFloat("TaxMoney");
    ...
    <tr class=listcontent id=TR2<%=i%>   align="center"  title="双击可登记代收费用"  onclick="changeclass1(this,'clickbg','listcontent');">
	  ...
	  <td  ><%=TaxName%></td>
	  <td  ><%=TaxMoney%></td>
       <td><img src="../images/edit.gif" border="0"  onmouseover="javascript:this.style.cursor='hand'" alt='修改' onclick="openwin('../finance/ModiTaxS.jsp?id=<%=id %>&taxmoneys=<%=TaxMoney %>&name=<%=TaxName %>&taxname_type=<%=taxtype1 %>&state=<%=state %>',300,300,100,100);" >&nbsp;<img src="../images/button_sc.gif"  border="0"   onmouseover="javascript:this.style.cursor='hand'" title="删除" onclick="deleteC(<%=id %>)";></td>
...
<%       List list1=new LinkedList();//存放栏名
		...
		List list8=new LinkedList();//实际栏名对应的数据
		  ...
		  ResultSet rs1=null;
			      ...
			      out.print("</tr>");
			      rs1.close();
		String c_ba="select a.contractno  from appraisal a ,order_contract  s  where s.section='"+SecNo+"' and s.loft='"+Loft+"' and s.room_no='"+RoomNo+"' and s.code*=a.contractno and s.state<>3 and s.cq=1 ";
		 ResultSet rsc_ba=ViewBean.executeQuery(c_ba);
		 while (rsc_ba.next()){
		    list4.add(rsc_ba.getString("contractno"));
		 }
