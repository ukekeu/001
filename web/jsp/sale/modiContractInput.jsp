...
<tr  valign="top">
    <td class="<%if(sel.equals("1")){out.print("menuselect3");}else{out.print("selemenu4");}%>" align="center" width=94 height=21>
	  <a href="#" onclick="document.cus.sel.value=1;findSub('modiContractInput.jsp');"><font color='<%if(sel.equals("1")){out.print("#FFFFFF");}else{out.print("#000000");}%>'>合同资料</font></a>
	</td>
	<td class="<%if(sel.equals("2")){out.print("menuselect3");}else{out.print("selemenu4");}%>" align="center" width=94 height=21>
	 <a href="#" onclick="document.cus.sel.value=2;findSub('modiContractInput.jsp');"><font color='<%if(sel.equals("2")){out.print("#FFFFFF");}else{out.print("#000000");}%>'>付款情况</font></a>
	</td>
	<td class="<%if(sel.equals("3")){out.print("menuselect3");}else{out.print("selemenu4");}%>" align="center" width=94 height=21>
	 <a href="#" onclick="document.cus.sel.value=3;findSub('modiContractInput.jsp');"><font color='<%if(sel.equals("3")){out.print("#FFFFFF");}else{out.print("#000000");}%>'>办证情况</font></a>
	</td>
	 <script>
...
<input type=hidden name="editnow"  　 value="1"><input type=button name=FindB  value="详情" onclick="xiangqing();"> 
	 ...
	 <table width="100%">
      ...
      <td class=roomright >&nbsp;<input type=text   name="zmyy"  size=50    >
...
<tr> 
   ...
   <td width="12%" class=roomright>&nbsp;<input type=text name="rgsno" value="<%=rgno %>"  size="10"   onChange="toUpper(document.cus.rgsno)" onKeyDown="key(document.cus.cale1)" onBlur="checkedOrderNoExist();" title="只能是英文字母、数字及"-"组成"></td>
	...
	</td> <td width="19%">  <a href="#"   onclick="openwin3('disc.jsp?code=<%=SecNo+Loft+RoomNo %>&discPrice=<%=room_price %>&discUnit=<%=saleJzPrice %>',700,400,100,100);"">添加/修改优惠</a> <br>
	   </td></tr>
	  <tr><td colspan="6"> 
...
<tr> 
 ...
 double s_price=0;
   ...
   <div id="ModiCus" style="display:none">
	 <table width="100%">
	  
      <td width="16%" class=roomleft>&nbsp;变更事项</td>
      <td class=roomright >&nbsp;更名<input name="choosetxt" type="checkbox" value=1>&nbsp;加名<input name="choosetxt" type="checkbox" value=2>
	  &nbsp;减名<input name="choosetxt" type="checkbox" value=3>&nbsp;调楼<input name="choosetxt" type="checkbox" value=4>
	  &nbsp;改付款方式<input name="choosetxt" type="checkbox" value=5>&nbsp;提前付款<input name="choosetxt" type="checkbox" value=6>
	  &nbsp;其他<input name="choosetxt" type="checkbox" value=7>&nbsp;<input type="button" value="打印申请表" name="chooseB" onclick="PrintChoose();">
        　</td>
  ...
  if ((loke.equals("1")&&nn>loke_time)||loke.equals("0")||loke_time==0){ %>
   ...
   <input type="button" value="变更申请" name="modicustomer" onclick="modiC();">
      &nbsp;&nbsp; 
   <input type="button" value="优惠申请表" name="discapp" onclick="openwin('DiscApp.jsp?code=<%=code %>');">
    &nbsp;&nbsp; 
	<input type="button" name="subsave" value=" 挞定审批表" onclick="openwin('OutOrder.jsp?code=<%=code %>');">&nbsp;&nbsp; 
    <input type="button" value="退 出" name="reset" onclick="window.close();">
		...
		<script>
		  function modiC(){
     	  document.all.ModiCus.style.display=(document.cus.modicustomer.value=='变更申请')?'block':'none';
		  if (document.cus.modicustomer.value=='变更申请')
		    {document.cus.modicustomer.value="取消变更";
			document.cus.submit1.disabled=true;
			}
		  else 	
		  {  document.cus.modicustomer.value='变更申请'
		  document.cus.submit1.disabled=false;
		    }
          }
		 function PrintChoose(){
		  var Num=document.cus.choosetxt.length;//当前checkbox的length
		  var CodeStr="";
		  for(var j=0;j<Num;j++)
		     {
			 if (document.cus.choosetxt[j].checked){
			  if  (CodeStr=="")CodeStr=document.cus.choosetxt[j].value;
			  else CodeStr=CodeStr+","+document.cus.choosetxt[j].value; 
		     }
		   }
		  if (CodeStr==""){
		    alert("请选择你变更事项");
		  }else{
		   openwin("PrintCusApp.jsp?CodeStr="+CodeStr+"&code=<%=code %>",500,450,10,10);
