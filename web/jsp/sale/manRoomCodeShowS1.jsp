   ...
   if (ssr.next()){
	   ...
	   <tr>
		...
		for(int j=intBeginR;j<=intEndR;j++){
		   ...
		   NO=AsiicZH(j,BeR);//调用asiic转换方法
			...
			%>
		      <td align="center" <%  if (Cortrol.indexOf("Y")>=0){ %> colspan="2"<%} %> class=listtitle width="5%"><font color="#ffffff"><%=NO%></font><!--<input type=checkbox name="Check<%=j%>" value="<%=j%>" checked onclick="LokeRoomNo('<%=strSection%>','<%=Loft %>','<%=j %>',this);">--></td>
			 <%}
			 }else if (!Lock.contains(String.valueOf(j))){//LockRoomS.indexOf(String.valueOf(j))<0
			 %>
			  <td align="center" <%  if (Cortrol.indexOf("Y")>=0){ %> colspan="2"<%} %> class=listtitle width="5%"><font color="#ffffff"><%=NO%></font></td>
			 <%} %>	
			 <%}
	...
	<%  if (Cortrol.indexOf("A")>=0){  %><a href="#<%=RoomNo %>" onclick="opennewwin('roomdata.jsp','?secname=<%=SecName %>&secno=<%=strSection%>&roomno=<%=no%>&build=<%=strBuilding%>&loft=<%=Loft %>&Floor=<%=i %>&state=1',720,530,120,5)"><%} %><%=RoomNo%><%  if (Cortrol.indexOf("A")>=0){  %></a><%} %>
	 ...
	 <%}else if (state.equals("2")){rgnum++;%>
	   <a href="#<%=RoomNo %>" title="如果未打印认购书,可修改认购书"  <%  if (Cortrol.indexOf("E")>=0){  %>  onclick="opennewwin('modiorder.jsp','?secname=<%=SecName %>&secno=<%=strSection %>&roomno=<%=no%>&build=<%=strBuilding%>&loft=<%=Loft %>&Floor=<%=i %>&show=false',750,500,100,10)"<%} %>><%=RoomNo%></a> 
	 ...
	 <%}else if (state.equals("6")){ %>
	  ...
	  <a href="#<%=RoomNo %>"  <%  if (Cortrol.indexOf("A")>=0){  %> onclick="opennewwin('roomdata1.jsp','?secname=<%=SecName %>&secno=<%=strSection %>&roomno=<%=no%>&build=<%=strBuilding%>&loft=<%=Loft %>&Floor=<%=i %>&state=5',750,530,120,5)"<%} %>><%=RoomNo%></a>
...
%>
	...
	<tr> 
	 ...
	 <% if (state.equals("1")){nosalenum++; %>
	  <%  if (Cortrol.indexOf("A")>=0){  %><a href="#<%=RoomNo%>" onclick="opennewwin('roomdata.jsp','?secname=<%=SecName %>&secno=<%=strSection%>&roomno=<%=no%>&build=<%=strBuilding%>&loft=<%=Loft %>&Floor=<%=i %>&state=1',720,530,120,5)"><%} %><%=RoomNo%><%  if (Cortrol.indexOf("A")>=0){  %></a><%} %>
	 <%}else if (state.equals("2")){rgnum++; %>
       <a href="#<%=RoomNo%>"  <%  if (Cortrol.indexOf("E")>=0){  %> onclick="opennewwin('ContractInput.jsp','?secname=<%=SecName %>&secno=<%=strSection %>&roomno=<%=no%>&build=<%=strBuilding%>&loft=<%=Loft %>&Floor=<%=i %>&show=true',720,530,120,2)"<%} %>><%=RoomNo%></a> 
	 <%}else if (state.equals("3")){salenum++;%>
	  <a href="#<%=RoomNo%>"  <%  if (Cortrol.indexOf("E")>=0){  %> onclick="opennewwin('ContractInput.jsp','?secname=<%=SecName %>&secno=<%=strSection %>&roomno=<%=no%>&build=<%=strBuilding%>&loft=<%=Loft %>&Floor=<%=i %>&show=true',720,530,120,2)"<%} %>><%=RoomNo%></a> 
	 <%}else if (state.equals("4")){yl++;%>
