...
<table width="98%" border="0" cellpadding="1" cellspacing="1">
  ...
  str+="<tr  align='center' >";//onmouseout='mOut(this,'#e6f9ff');' onmouseover='mOvr(this,'#EAECEC');'
    ...
    if (state1){//如果是固定金额
      if (state4){//楼款
	    if (state3)
		 str+="<td align=left width='49%'><input type=text name=a1 class=unnamed1 value="+qs+""+aa+""+scale+"(扣除定金)></td>";
		else 
	  ...
	  if (state3)
	   if (SaleSumPrice.equals("1"))
         str+="<td align=left width='49%'>"+qs+"<input type=text name=gw class=unnamed1 value="+aa+"&nbsp;&nbsp;&nbsp;&nbsp;(扣除定金)></td>";
	   else
	     str+="<td align=left width='49%' >"+qs+"<input type=text name=gw class=unnamed1  value="+aa+""+format.getFloat2(Double.parseDouble(SaleSumPrice)*scale/100+Double.parseDouble(ykc)-Double.parseDouble(dj),2)+">元</td>";
	  else
        if (SaleSumPrice.equals("1"))
...
%>
<tr><td align=left width='49%' >认购付定金:<input type=text name=gw class=unnamed1  value='<%=dj %>'>元</td>

<td align=left >&nbsp;付款时间:<input type=text name=gw class=unnamed1 value="签定认购书时">付清</td>
</tr>
<%
