...
<script>
  ...
  <table width="90%" border="0" CELLSPACING=1 CELLPADDING=3>
   <tr> 
		<td width="100" class=listtitle align="center"><font color="#FFFFFF"<b>楼层\房号</b></font></td>
		<%
	...
	String sqlArr="select   Room_code,saleState,Floor,Room,area,sumprice,floorshow,floortype,floormess from roomstateview where seccode=";
		   sqlArr=sqlArr+"'"+strSection+"'";
		   sqlArr=sqlArr+FindStr+" and floortype=0   order by Floor";
	...
	<%  if (Cortrol.indexOf("A")>=0){  %> <input type=checkbox name=yl value="<%=RoomNo %>"><a href="#" onclick="opennewwin('roomdata.jsp','?secname=<%=SecName %>&secno=<%=strSection%>&roomno=<%=no%>&build=<%=strBuilding%>&loft=<%=Loft %>&Floor=<%=i %>&state=1',680,530,120,5)"><%} %><%=RoomNo.substring(RoomNo.indexOf("-")+1)%><%  if (Cortrol.indexOf("A")>=0){  %></a><%} %>
	  <%}else if (state.equals("3")){ salenum++;%>
       <%=RoomNo.substring(RoomNo.indexOf("-")+1)%> 
	 <%}else if (state.equals("2")){rgnum++;%>
	   <a href="#" title="如果未打印认购书,可修改认购书" onclick="opennewwin('modiorder.jsp','?secname=<%=SecName %>&secno=<%=strSection %>&roomno=<%=no%>&build=<%=strBuilding%>&loft=<%=Loft %>&Floor=<%=i %>&show=false',690,500,100,10)"><%=RoomNo%></a> 
	
	
	 <SELECT NAME=PR title="请选择打印类型" onchange="printrgs(this.value,'secno=<%=strSection %>&roomno=<%=no%>&build=<%=strBuilding%>&loft=<%=Loft %>&Floor=<%=i %>');"><option >打印</option><option value=1>HP</option><option value=2>LQ</option></select>
	  
	 <%}else if (state.equals("4")){yl++;%>
	<%  if (Cortrol.indexOf("Y")>=0){  %>   <input type=checkbox name=yl value="<%=RoomNo %>"><a href="#" onclick="choose('<%=RoomNo%>');ylroom();"><%} %><%=RoomNo.substring(RoomNo.indexOf("-")+1)%><%  if (Cortrol.indexOf("A")>=0){  %></a><%} %>
...
%>
	 ...
	 <script>w1<%=i %>.innerHTML="<% if (!floormess.equals("")) out.print(floormess);else out.print(i);%>";</script>
	...
	<%  if (Cortrol.indexOf("A")>=0){  %> <input type=checkbox name=yl value="<%=RoomNo %>"><a href="#" onclick="opennewwin('roomdata.jsp','?secname=<%=SecName %>&secno=<%=strSection%>&roomno=<%=no%>&build=<%=strBuilding%>&loft=<%=Loft %>&Floor=<%=i %>&state=1',680,530,120,5)"><%} %><%=RoomNo.substring(RoomNo.indexOf("-")+1)%><%  if (Cortrol.indexOf("A")>=0){  %></a><%} %>
	 <%}else if (state.equals("2")){rgnum++; %>
       <a href="#" onclick="opennewwin('modiorder.jsp','?secname=<%=SecName %>&secno=<%=strSection %>&roomno=<%=no%>&build=<%=strBuilding%>&loft=<%=Loft %>&Floor=<%=i %>&show=false',690,500,100,10)"><%=RoomNo.substring(RoomNo.indexOf("-")+1)%></a> 
	 <SELECT NAME=PR title="请选择打印类型" onchange="printrgs(this.value,'secno=<%=strSection %>&roomno=<%=no%>&build=<%=strBuilding%>&loft=<%=Loft %>&Floor=<%=i %>');"><option >打印</option><option value=1>HP</option><option value=2>LQ</option></select>
