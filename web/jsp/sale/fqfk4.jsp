...
<table border="0" width="99%"  cellpadding="0" cellspacing="0" style="border-collapse: collapse" bordercolor="#111111" width="100%" id="AutoNumber1">
  <tr align="center"><td>现金一次性收款方式月度明细报表</td></tr>
  <tr><td>&nbsp;</td></tr>
...
<table border="1" width="99%"  cellpadding="0" cellspacing="0" style="border-collapse: collapse" bordercolor="#111111" width="100%" id="AutoNumber1">
  ...
  while (rs.next()){
   ...
   SQL+="a.ajtxdate,a.VisaDate), 120) ajdate,a.rmbprice kk,a.code FROM order_contract a ,customer b WHERE a.customer=b.serialno and  a.state=2 and convert(char(10),a.date,120)>='"+stdate+"' and convert(char(10),a.date,120)<='"+endate+"' and a.section='"+SecNo+"' and a.payment1 like '%一次%' and  a.YONG_TU='"+rs.getString("YONG_TU")+"' order by a.loft+a.room_no";
...
<td width="7%"><%=rs1.getString("kk") %></td>
    <td width="8%"  align="center"><%=rs1.getString("visadate") %></td>
    <td width="8%"><%=FormtD.getFloat2(rs1.getDouble("dj"),2) %></td>
    <td width="8%"  align="center"><%=rs1.getString("visadate") %></td>
	<%ysk=rs1.getDouble("kk") ; sumprice+=ysk;%>
    ...
    <td width="8%"><%=FormtD.getFloat2(ysk-ljsk,2) %></td>
    <td width="8%">　</td>
  </tr> 
  <%ii++;}
  rs1.close(); 
