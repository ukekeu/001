...
<%!
  ...
  <%
   ...
   String Cortrol=checke_qx(ViewBean,(String)session.getAttribute("loginid"),"QT022");
		...
		sql="select Top "+pageSize+"  *,datediff(d,enddate,getdate()) day from View_Rent_List where Code not in (Select top "+prenum +" code from View_Rent_List where  ht_state=0 "+find+stdate+FindWhere +findStr +orderStr+")  and   ht_state=0 "+find+stdate+FindWhere +findStr +orderStr;
		sqlcount="select count(*) as num from View_Rent_List where ht_state=0   " +find+stdate+FindWhere + findStr ;
 
%>
<body topmargin="1" >
<form action="rent_list.jsp" method="post" name="frm">
