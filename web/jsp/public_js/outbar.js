...
{ Folder=eval("OutBarFolder"+i)
 window.status="正在建立仿 Outlook 菜单条 '"+Folder[0]+"'";
 ...
 if(i==1)
 {
  ...
  document.write("<INPUT position='UP' title='单击展开菜单"+"' id='OB_Button1' onDblClick='OutlookLikeBar.FolderClicked("+i+");this.blur()' onClick='OutlookLikeBar.FolderClicked("+i+",this.value);this.blur();' TYPE='button' value='"+titlemess+"' style='position:absolute;top:0;left:0;width:90;height:"+OB_ButtonHeight+";font-family:"+OB_ButtonFontFamily+";font-size:"+OB_ButtonFontSize+"pt;cursor:hand;color:"+OB_ButtonFontColor+";z-index:100'>");
  MakeItems(Folder,i,OB_ButtonHeight);		
 }	
 else
 {
  document.write("<INPUT position='DOWN' title='单击展开菜单"+"'  id='OB_Button"+i+"' onDblClick='OutlookLikeBar.FolderClicked("+i+");this.blur()' onClick='OutlookLikeBar.FolderClicked("+i+",this.value);this.blur();' TYPE='button' value='"+titlemess+"' style='position:absolute;top:"+(OB_Height-(j-i)*OB_ButtonHeight-OB_BorderWidth*2)+";left:0;width:90;height:"+OB_ButtonHeight+";font-family:"+OB_ButtonFontFamily+";font-size:"+OB_ButtonFontSize+"pt;cursor:hand;color:"+OB_ButtonFontColor+";z-index:100'>");
...
function MakeItems(Folder,zorder,top)
{
 ...
 for(var i=1;i<items*4;i+=4)
 {
  ...
  document.write("<img src='"+Folder[i]+"' title ='"+show+"' onclick=window.status='当前操作项："+menutitle+"→【"+menutitle1+"　　】　　　当前用户:【"+username+time+"】'>");
  document.write("</div>");
...
{	 
   ...
   window.status=username+" ，您已进入管理信息系统,当前模块:【"+para+"】";
...
window.status=username+"，您已进入管理信息系统";