   ...
   <%  int id=0;
      int u = 0;
	   ...
	   String code="";
	    ...
	    ResultSet rsc = aBean2.executeQuery(" select company from CortrolMan group by company");
  while(rsc.next()){
	 ...
	 <td colspan=8>  <%=company %> </td>
	    ...
	    ResultSet rs = aBean.executeQuery("select dept from CortrolMan  where company='"+company+"' group by dept");
   ...
   while(rs.next()){
	...
	<%
   ResultSet rs1 = aBean1.executeQuery(" select * from CortrolMan where dept='"+dept+"' and company='"+company+"'");
...
String state1=getbyte(rs1.getString("state"));
	...
	<tr class='listcontent'  id=tr<%=u %>>
	 ...
	 <td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>
	 <td><input type=checkbox name=ic<%= u%> value="<%=name %>" ><%= serialno%></td>	
...
function choosevalue() {
     ...
     for (i=0;i<form1.elements.length;i++) {
	   ...
	   opener.document.pm.object.value=seldocs;
	   window.close();
}	
