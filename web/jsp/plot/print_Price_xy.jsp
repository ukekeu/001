...
<% 
	...
	if (ok.equals("ok")){
	  String SQL="select * from  试算价目表 where paymenttype=1 and  sectionno='"+SecNo+"' and loft='"+Loft+"' and payment='"+payment+"' and building='"+Build+"'";
	  ResultSet rs1 = aBean.executeQuery(SQL);
	   ...
	   while (rs1.next()){
	     ...
	     String sql="Update room set xy_jzprice="+unitprice+",";
		 sql+="xy_Sumprice="+price;
		 sql+=",xy_TNprice1="+tnunitprice;
		 sql+=" where seccode='"+SecNo+"' and cell='"+cell+"' and loft='"+Loft+"' and room_code='"+roomno+"' and building='"+Build+"'";
	     aBean1.executeUpdate(sql);
		 
	   }
	     aBean1.executeUpdate(" update 试算价目表 set state=0 where paymenttype=1 and  sectionno='"+SecNo+"' and loft='"+Loft+"' and building='"+Build+"'");
	   aBean1.executeUpdate(" update 试算价目表 set state=1,checkstate=1,userstate=1 where paymenttype=1 and  sectionno='"+SecNo+"' and loft='"+Loft+"'and building='"+Build+"'  and payment='"+payment+"'");
	 %>
	<script>
    alert("系统已传递价格!");	
	...
	<%
	  ...
	  for (int a=1;a<=java.lang.Integer.parseInt(num);a++){
		...
		if (!jzunitprice.equals("")){
			...
			modiSQL+=" where paymenttype=1 and  sectionno='"+SecNo+"' and loft='"+Loft+"'and building='"+Build+"' and payment='"+payment+"' and roomno='"+roommo+"'";
...
function sendtoprice(){
  ...
  document.frm.action="print_Price_xy.jsp?ok=ok&SecNo=<%=getbyte2(request.getParameter("SecNo")) %>&Loft=<%=getbyte2(request.getParameter("Loft")) %>&payment=<%=payment %>";
  document.frm.submit();
  }
...
function savemodi(){
 if (confirm("确认保存当前的修改吗？")){
  ...
  document.frm.action="print_Price_xy.jsp?ok=modi&SecNo=<%=getbyte2(request.getParameter("SecNo")) %>&Loft=<%=getbyte2(request.getParameter("Loft")) %>&payment=<%=payment %>&num="+document.frm.num.value;
  document.frm.submit();
  }
...
<br>
 <table width="80%"  border=1 cellspacing='0'  cellpadding='0' bgcolor=ffffff bordercolordark=ffffff bordercolorlight=000000>
  ...
  <%
   ...
   int num2=0;
   String code="";
     String sql= "select a.* from  试算价目表 a,room b where a.paymenttype=1   and a.sectionno+a.loft+a.roomno=b.seccode+b.loft+b.room_code and a.sectionno='"+SecNo+"' and a.loft='"+Loft+"' and payment='"+payment+"'";
 ...
 <td  align="center"><%=rs.getString("roomno") %></td>
  <td  align="center"><%=rs.getString("area1") %></td>
   <td  align="center"><input type=text size=10 name=jzprice<%=ii %> onkeydown="key(document.frm.jzprice<%=ii+1 %>);"  value="<%=rs.getString("price1") %>" onchange="document.frm.sumprice<%=ii %>.value=parseFloat(document.frm.jzprice<%=ii %>.value)*<%=rs.getString("area1") %>;"></td>
    <td  align="center"><input type=text size=10  readonly name="sumprice<%=ii %>" value="<%=FormatD.getFloat1(rs.getFloat("sumprice"),0) %>"></td>
     </tr  > 
   <% 
...
%> 
	  ...
	  <input type=hidden name=num value="<%=ii %>">
	   <%sql="SELECT yong_tu,sum(area1) area,sum(sumprice) /sum(area1) avg,sum(sumprice) sumprice ,count(*) num from 试算价目表 where paymenttype=1 and  sectionno='"+SecNo+"' and loft='"+Loft+"' group by yong_tu";
 ...
 int iii=0;
   ...
   <tr><td align="center"><%=yongtu%></td>
	 ...
	 <td align="center"><%=FormatD.getFloat(rs.getFloat("sumprice"),0) %></td>
