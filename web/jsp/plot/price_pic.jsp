...
<% 
	...
	if (ok.equals("ok")){
	  String SQL="select * from  试算价目表 where sectionno='"+SecNo+"' and loft='"+Loft+"' and payment='"+payment+"' and building='"+Build+"'";
	  ResultSet rs1 = aBean.executeQuery(SQL);
	   ...
	   while (rs1.next()){
		 ...
		 sql+=" where seccode='"+SecNo+"' and loft='"+Loft+"' and room_code='"+roomno+"' and building='"+Build+"'";
	     aBean1.executeUpdate(sql);
		 
	   }
	   aBean1.executeUpdate(" update 试算价目表 set state=0 where sectionno='"+SecNo+"' and loft='"+Loft+"' and building='"+Build+"'");
	   aBean1.executeUpdate(" update 试算价目表 set state=1,userstate=1 where sectionno='"+SecNo+"' and loft='"+Loft+"'and building='"+Build+"'  and payment='"+payment+"'");
	...
	<%
	  ...
	  for (int a=1;a<=java.lang.Integer.parseInt(num);a++){
		...
		if (!jzunitprice.equals("")){
			...
			modiSQL+=" where sectionno='"+SecNo+"' and loft='"+Loft+"'and building='"+Build+"' and payment='"+payment+"' and roomno='"+roommo+"'";
...
&nbsp;<input type="button" name="Button1" value="传递价格" onClick="sendtoprice();"> 
...
%> 
	  ...
	  <input type=hidden name=num value="<%=ii %>">
	   <%sql="SELECT yong_tu,sum(area1) area,sum(sumprice) /sum(area1) avg,sum(sumprice) sumprice ,count(*) num from 试算价目表 where sectionno='"+SecNo+"' and loft='"+Loft+"' group by yong_tu";
