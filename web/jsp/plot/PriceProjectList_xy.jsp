...
<HTML>
<HEAD>
<TITLE>虚拟价格方案管理</TITLE> <META HTTP-EQUIV="Content-Type" CONTENT="text/html; chrset=gb2312"> 
...
function findSub(para){
 ...
 String room="A";
  ...
  String str="select payment from  试算价目表 where paymenttype=1 and sectionno='"+SecNo+"' and loft='"+Loft+"' and payment='开盘虚拟价格'";
     int aaa=0;
...
Cortrol=checke_qx(aBean,(String)session.getAttribute("loginid"),"JC003");
  ...
  if (!room.equals("")) { 
    %>
	    ...
	    <table width=100%><tr height="16"  ><td align="right"> 
		 ...
		 <input type=button name=pri value="从原真实价格方案导入"  <%if (aaa>0)out.print("disabled"); %> onclick="AddP();" >
		<input type=button name=pri value="虚拟定价"   onclick="Add();">
		<%} %>
  ...
  <tr  class=TXTColor>
	...
	<%  str="select payment,convert(varchar(10),date,120) date,state,checkstate,userstate from  试算价目表 where paymenttype=1 and sectionno='"+SecNo+"' and loft='"+Loft+"' group by convert(varchar(10),date,120),payment,state,checkstate,userstate order by date";
   ...
   %>
    <tr  class='listcontent'id=TR<%=i %>  align="center"    onmouseover="mout(this);" onclick="ChangeTrColor(this, '<%= id %>', '<%= id %>', '<%= id %>', 'A6D3A6', 'f6f6f6');changeclass1(this,'clickbg','listcontent');"   > 
      ...
      <TD WIDTH="30%"  > 
        <DIV ALIGN="CENTER">
		<%if (states==1)out.print("<font color=red>目前采用价格</font>");
		 if (Cortrol.indexOf("C")>=0){  
		 %>
		<a href="#" onclick="openwin('print_Price_xy.jsp?SecNo=<%=SecNo %>&Loft=<%=Loft %>&Build=<%=Build %>&payment=<%=name  %>&state=<%=states %>')">启用</a>
		<%} %>
		 <a href="#" onclick="openwin('xy_PriceListView.jsp?SecName=<%=SecName %>&SecNo=<%=SecNo %>&Loft=<%=Loft %>&Build=<%=Build %>&payment=<%=name  %>&state=<%=states %>')">预览</a>
		</DIV>
		 </TD>
	    <TD WIDTH="20%" > 
		<%if (checkstate==0&&states==0) {%>
			<a  href="#" onclick="ChangeTrColor(this, '<%= id %>', '<%= id %>', '<%= id %>', 'A6D3A6', 'f6f6f6');Modify();">发送审批</a>
		...
		<a href="#" onclick="openwin('delete_Price.jsp?SecNo=<%=SecNo %>&Loft=<%=Loft %>&Build=<%=Build %>&payment=<%=name  %>',10,10)">删除</a>
	   <%} %>
      </TD>
    </TR>
    <%
   ...
   <%
	...
	String filename = String.valueOf(calendar.getTimeInMillis());	    
      str="select  payment,convert(numeric(9,0),str(avg(price1))) price,convert(varchar(10),date,120) date,state from  试算价目表 where paymenttype=1 and sectionno='"+SecNo+"'  and ( building='"+Build+"'or building is null) ";
	 if (!Loft.equals("")){str+="and loft='"+Loft+"'";
	 str+="group by convert(varchar(10),date,120),payment,state order by date ";
	 pic=FormtD1.main1("D:/"+ request.getContextPath()+"/crmv9/jsp/picture/adjricefx"+filename+".jpg","虚拟价格方案均价变化分析","均价(元)","价格方案均价变化分析",str,1,"预售均价","","","");
