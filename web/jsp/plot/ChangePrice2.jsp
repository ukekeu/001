...
if (ecell<0)ecell=0;
	...
	String zjrate="",Rinfo="";
	if (!OK.equals("")){
	changePriceListBean.executeUpdate("delete 试算价目表 where payment='"+paym+"' and sectionno='"+section+"' and loft='"+Region+"'");
	     ...
	     for(int j=Integer.parseInt(intBeginR);j<=Integer.parseInt(intEndR);j++){
		   ...
		   String str1="P"+Rinfo+String.valueOf(i)+String.valueOf(j);
	        ...
	        if (!str1.equals("")){
			  ...
			  if (RoomNos.equals("")){
			   RoomNos="'"+roomcode+"'";//取出选中的值(房间号)
			   RoomRate=" update room set "+StrPriceName+"="
			   +" case when (room_code='"+roomcode+"') "
			   +" then " +str1;
				StrNowPrice1="  when (room_code='"+roomcode+"') "
			    +" then " +str1+ StrAreaType;
				StrNowPrice2="  when (room_code='"+roomcode+"') "
			    +" then " +str1;
			   ...
			   RoomRate+=" when (room_code='"+roomcode+"') "
			   +"then " +str1;
			   //"Now"+StrPriceName1+"="
			   //现套内单价
			    StrNowPrice1+="  when (room_code='"+roomcode+"') "
				...
				if (djtype.equals("zj")){
				//现总价
				NowSumPrice+="  when (room_code='"+roomcode+"') "
			    +" then " +str1;
				//现建筑单价
...
int AllSumNum=0;
  ...
  ResultSet change2RS = null;
	try{
		...
		while (change2RS.next()) {
		  ...
		  <%
		   if (!StrFloors.equals("")&&!StrFloors.equals(floor)){//如果不属于同一楼层，则输出上一楼层的小计
		     ...
		     out.print("<td  colspan=2><font color=red>建筑面积:"+FormatD.getFloat(FatSumArea,2)+"</font></td>");
	         out.print("<td  colspan=2><font color=red>总金额:</font><font color=red>"+FormatD.getFloat(IntSumPrice,0)+"</font></td>");
		     out.print("<td colspan=2><font color=red>均价:"+FormatD.getFloat((IntSumPrice)/FatSumArea,0)+"</font></td>");
			 out.print("</tr>");
		...
		%>
		    ...
		    String NowsumPrice = FormatD.getFloat3(IntPrvSumPrice,0);
 ...
 %>
    ...
    <tr align="right"  bgcolor="#DDE8E8"> 
      ...
      <td ><%= tnunitPrice %></td>
	   <td ><%= sumPrice %></td>
      <td  bgcolor="#E8DEDE"><%=FormatD.getFloat2(Double.parseDouble(Nowunitprice),2) %></td>
      <td  bgcolor="#E8DEDE"><%= FormatD.getFloat2(Double.parseDouble(NowtnunitPrice),2) %></td>
      <td  bgcolor="#E8DEDE"><%= FormatD.getFloat2(IntPrvSumPrice,0) %></td>
    </tr>
  ...
  <%	IntRoomNum++;
   ...
   String ss="insert into 试算价目表 select '"+paym+"','";
		 ss+=section+"','"+Region+"','"+room+"',"+roomn+","+floor+","+NowtnunitPrice+",";
		 	ss+=Nowunitprice+","+AreaStr+","+jzarea+","+NowsumPrice+",getdate(),'"+yong_tu+"',0,'"+building+"','"+cell+"',0,0,0";
...
<script>
opener.window.location="PriceProjectList.jsp?SecNo=<%=section %>&Loft=<%=Region %>";
function save(){
