...
function checkedPaymentNoExist(){
 ...
 function Validate(){
    ...
    document.frm.action="ChangePrice2.jsp?Ecellname=<%=Ecellname %>&Scellname=<%=Scellname %>";
	document.frm.submit();
  }
...
<table width="100%" border="0">
  ...
  <table width="100%" border="0" cellspacing="1" cellpadding="1"  onMouseOver="MM_showHideLayers('ebusiness','','show');" onMouseOut="MM_showHideLayers('ebusiness','','hide')">
   ...
   <% if (state.equals("1")||state.equals("4")){%>
	...
	<% }}%>
	 <%
   sql="select count(*) nn,sum(area) area,sum(Sumprice) Sumprice from room where seccode='"+strSection+"' and SaleState in(1,4) and loft='"+Region+"' and building='"+strBuilding+"'";
  try{
