...
try{
  ...
  <%
   sql="select count(*) nn,sum(area) area,sum(Sumprice) Sumprice,SaleState from room where seccode='"+strSection+"' and SaleState in (1,4) and loft='"+Region+"' and building='"+strBuilding+"'"+"group by SaleState ";
  try{
...
<input type="hidden" name="AllUnitArea_S" value="<%=FormtD.getFloat(AllSumPrice_S/AllSumArea_S,2) %>">
  ...
  <div id="div2" <%if(!page.equals("2")){%>style="display:none"<%}%>>
	...
	<%
		 for(int j2=scell;j2<=ecell;j2++){
			...
			String	sqlArr="select  distinct Room_code,saleState,Floor,Room,area1,Sumprice,roomtype,direction,floormess from roomstateview where seccode=";
		   sqlArr=sqlArr+"'"+strSection+"'";
		   sqlArr=sqlArr+" and  loft='"+Region+"' and cell='"+Rinfo+"'  and building='"+strBuilding+"'  and SaleState in (1,4) "+floortype+" order by  Floor";
	...
	for(iii=0;iii<roomint;iii++){
	 //  if (iii==0) continue;
	   ...
	   if(state2.equals("1")||state2.equals("4")){%>
	<tr>
  ...
  <%} %>
   ...
   <%
	...
	for(iii=0;iii<roomint;iii++){
	   ...
	   String Fn=(String)list3.get(iii);
	    if (Fn.indexOf("-")>=0) Fn="_"+Fn.substring(Fn.indexOf("-")+1);
	   if(state3.equals("1")||state3.equals("4")){%>
	<tr>
	 <td align="center"  class=listcontent ><%= dda%> </td>
  ...
  <%} %>
	...
	for(iii=0;iii<roomint;iii++){
	   ...
	   String Fn=(String)list3.get(iii);
	    if (Fn.indexOf("-")>=0) Fn="_"+Fn.substring(Fn.indexOf("-")+1);
	   if(state4.equals("1")||state4.equals("4")){%>
	<tr>
	 <td align="center"  class=listcontent ><%= dda%></td>
  ...
  <%} %>
	...
	<%int aaa=0; 
	   ...
	   String Fn=(String)list3.get(iii);
	    if (Fn.indexOf("-")>=0) Fn="_"+Fn.substring(Fn.indexOf("-")+1);
	   if(state5.equals("1")||state5.equals("4")){%>
	<tr>
 ...
 // opener.window.location.reload();
  ...
  function Validate(){
   if (document.frm.projectname.value=="")
    ...
    document.frm.action="PriceList2.jsp?Ecellname=<%=Ecellname %>&Scellname=<%=Scellname %>&allint=<%=roomint %>&loftnum=<%=loftnum/ecell %>";
	document.frm.submit();
  }
   function Validate1(){
    if (document.frm.projectname.value=="")
