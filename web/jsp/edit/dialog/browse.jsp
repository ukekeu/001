...
document.write("<title>文件浏览(" + sTitle + ")</title>");
...
function setFolderList(a_Upload, a_ShareImage, a_ShareFlash, a_ShareMedia, a_ShareOther){
	...
	switch(sType){
	case "image":
		html += getFolderTree(a_ShareImage, true, "shareimage", "共享图片库");
		break;
	case "flash":
		html += getFolderTree(a_ShareFlash, true, "shareflash", "共享Flash库");
		break;
	case "media":
		html += getFolderTree(a_ShareMedia, true, "sharemedia", "共享媒体库");
		break;
	default:
		html += getFolderTree(a_ShareImage, false, "shareimage", "共享图片库");
		html += getFolderTree(a_ShareFlash, false, "shareflash", "共享Flash库");
		html += getFolderTree(a_ShareMedia, false, "sharemedia", "共享媒体库");
		html += getFolderTree(a_ShareOther, true, "shareother", "共享的文件");
		break;
	}
