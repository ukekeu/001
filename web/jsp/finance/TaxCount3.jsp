...
// out.print(SQL);
 while (countRs.next()){
   loft=countRs.getString("roomno");
	...
	ViewBean3.executeUpdate("delete zy_count_Charges where ContactNo="+code);
	...
	if (rh){//只读取需要入户客户应交的费
	 //  if (custype.equals("0")){
		 ...
		 ShowTax("select t.id,t.code,t.money,t.termvalue,t.term,t.termvalue1,t.term1,t.termvalue2,t.term2,t.termvalue3,t.term3,t.money1,t.state,t1.tax_name,t1.tax_type,t1.type2,t.type1,t.type3,t.type from TaxChages t,tax t1 where t.code=t1.code and t.state='1' and t.type="+custype+" and t.code  in (select Code from tax where type2=0 and type=2 and  type1='所有' and section='"+section+"')",httype); 
	     ShowTax("select t.id,t.code,t.money,t.termvalue,t.term,t.termvalue1,t.term1,t.termvalue2,t.term2,t.termvalue3,t.term3,t.money1,t.state,t1.tax_name,t1.tax_type,t1.type2,t.type1,t.type3,t.type from TaxChages t,tax t1 where t.code=t1.code and t.state='2' and t.type="+custype+" and t.code  in (select Code from tax where type2=0 and type=2 and  type1='所有' and section='"+section+"')",httype); 
		 ShowTax("select t.id,t.code,t.money,t.termvalue,t.term,t.termvalue1,t.term1,t.termvalue2,t.term2,t.termvalue3,t.term3,t.money1,t.state,t1.tax_name,t1.tax_type,t1.type2,t.type1,t.type3,t.type from TaxChages t,tax t1 where t.code=t1.code and t.state='3' and t.type="+custype+" and t.code  in (select Code from tax where type2=0 and type=2 and  type1='所有' and section='"+section+"')",httype);
		 ShowTax("select t.id,t.code,t.money,t.termvalue,t.term,t.termvalue1,t.term1,t.termvalue2,t.term2,t.termvalue3,t.term3,t.money1,t.state,t1.tax_name,t1.tax_type,t1.type2,t.type1,t.type3,t.type from TaxChages t,tax t1 where t.code=t1.code and t.state='4' and t.type="+custype+" and t.code  in (select Code from tax where type2=0 and type=2 and  type1='所有' and section='"+section+"')",httype);
	     ShowTax("select t.id,t.code,t.money,t.termvalue,t.term,t.termvalue1,t.term1,t.termvalue2,t.term2,t.termvalue3,t.term3,t.money1,t.state,t1.tax_name,t1.tax_type,t1.type2,t.type1,t.type3,t.type from TaxChages t,tax t1 where t.code=t1.code and t.state='6' and t.type="+custype+" and t.code  in (select Code from tax where type2=0 and type=2 and  type1='所有' and section='"+section+"')",httype);
         ShowTax("select t.id,t.code,t.money,t.termvalue,t.term,t.termvalue1,t.term1,t.termvalue2,t.term2,t.termvalue3,t.term3,t.money1,t.state,t1.tax_name,t1.tax_type,t1.type2,t.type1,t.type3,t.type from TaxChages t,tax t1 where t.code=t1.code and t.state='7' and t.type="+custype+" and t.code  in (select Code from tax where type2=0 and type=2 and  type1='所有' and section='"+section+"')",httype);
         ShowTax("select t.id,t.code,t.money,t.termvalue,t.term,t.termvalue1,t.term1,t.termvalue2,t.term2,t.termvalue3,t.term3,t.money1,t.state,t1.tax_name,t1.tax_type,t1.type2,t.type1,t.type3,t.type from TaxChages t,tax t1 where t.code=t1.code and t.state='8' and t.type="+custype+" and t.code  in (select Code from tax where type2=0 and type=2 and  type1='所有' and section='"+section+"')",httype);
         ShowTax("select t.id,t.code,t.money,t.termvalue,t.term,t.termvalue1,t.term1,t.termvalue2,t.term2,t.termvalue3,t.term3,t.money1,t.state,t1.tax_name,t1.tax_type,t1.type2,t.type1,t.type3,t.type from TaxChages t,tax t1 where t.code=t1.code and t.state='9' and t.type="+custype+"  and t.code  in (select Code from tax where type2=0 and type=2 and  type1='所有' and section='"+section+"')",httype);
...
String qsSQL="delete zy_count_Charges where TaxName in (select TaxName from zy_count_Charges where ContactNo="+code+" group by TaxName having count(TaxName)>1) and ContactNo="+code+" and taxtype<>"+custype;
ViewBean3.executeUpdate(qsSQL);
 
ResultSet rs=ViewBean.executeQuery("select TaxMoney, TaxMoney/"+FormatD.getFloat3(SalePrice,2)+" qtaxrate from zy_count_Charges  where ContactNo="+code +" and TaxName like'%契税%'");
 if (rs.next()){
...
ViewBean3.executeUpdate("update order_contract set q_tax="+FormatD.getFloat3(Double.parseDouble(qtaxrate)*SalePrice,2)+", q_tax_rate="+qtaxrate+",q_tax_up='"+upperMoney+"',yh_tax="+yhtax+" where code="+code);
 ...
 String sql="select sum(money) TaxMoney,TaxName from factzy_count_Charges where contractno ='"+code+"' group by TaxName";//order by room_no desc";
    rs=ViewBean.executeQuery(sql);
   
  while (rs.next()){
     String modiTaxstate="update  zy_count_Charges  set state=case when TaxMoney>"+rs.getString("TaxMoney")+" then 0 else 1 end,factMoney="+rs.getString("TaxMoney")+" where taxname='"+rs.getString("TaxName")+"' and ContactNo="+code;
...
<script>
  window.location="../finance/yingshiList_s.jsp?contractno=<%=request.getParameter("contractno") %>&cusno=<%=request.getParameter("cusno") %>&pay=<%=request.getParameter("pay ")%>&custype=<%=request.getParameter("custype ") %>&ldroomcode=<%=request.getParameter("ldroomcode ")%>";
