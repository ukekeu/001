...
int id=0;
 ...
 if (defaultNo==null)defaultNo="";//如何用户未设置默认楼盘，则显示用户选中或所有楼盘的楼栋
 if (!defaultNo.equals(""))
   {SectionS=defaultNo;
   SecNo=defaultNo;
   }
		  ...
		  String BuildSql="select Build FROM sectiontree where SerialNo='"+StrCodeArr[jj]+"' and Loft='"+Loft+"' group by Build";
		   ...
		   try{
			 ...
			 id++;
			   ...
			   out.print("&nbsp;");
			   %>
			   <IMG height=13 name="filebox<%=id %>"  value="<%=id %>"  src="../picture/dot_01.gif" width=10 border=0  onclick="aad1(filebox<%=id %>,17);">
			   <a href="FactGatherDetail.jsp?SecNo=<%=StrCodeArr[jj] %>&Loft=<%=Loft %>&SecName=<%=StrNameArr[jj]%>&Build=<%=Building %>" target="frmright" onclick="aad1(filebox<%=id %>,100);" title='单击后在右边定义房号'>
			 <!--  <font color=white >-->
			   <%
			   out.print(Building+"&nbsp;");
		  ...
		  String BuildSql="select Build FROM sectiontree where SerialNo='"+StrCodeArr[n]+"' and loft='"+Loft+"' group by Build";
		   try{
			 ...
			 id++;
			   ...
			   <a href="../afterservice/AjList2002.jsp?SecNo=<%=StrCodeArr[n] %>&Loft=<%=Loft %>&SecName=<%=StrNameArr[n] %>&Build=<%=Building %>" target="frmright"  onclick="aad1(filebox<%=id %>,17);"  title='单击后在右边定义房号'>
			   <%
			   out.print(Building+"&nbsp;");
			     out.print("</a>");
			     out.print("</td ></tr>");
