...
<% 
 ...
 <td id="buil_<%=Loft%>"   onclick="changeclassname(this,'BL2','L2');">
  <img src="../images/document1.gif" border=0 align="absmiddle" hspace="4" ><a href="RoomCodeShow.jsp?SecNo=<%=SectionSerialNo %>&Loft=<%=Loft %>&SecName=<%=SectionName%>&checkQX=<%=checkQX %>" target="frmright"onclick="aad2(s<%=num %>,200);"><%=Loft %></a><img src="../images/h.gif"  name="s<%=num %>"  value="<%=num %>" >
  </td>
  </tr>
   <% 
 ...
 <img src="../images/tree.gif" border=0 align="absmiddle" hspace="4" ><a href="RoomCodeShow.jsp?SecNo=<%=SectionSerialNo %>&Loft=<%=Loft %>&SecName=<%=SectionName%>&Build=<%=Build %>" target="frmright"  title='单击后在右边查看付款情况' onclick="aad2(s<%=num %>,200);"><%=Build %></a><img src="../images/h.gif"  name="s<%=num %>"  value="<%=num %>" >
