...
<P align="center"><font size=4><b>退款登记</b></font></P>
 ...
 <table width="99%" border="0" CELLSPACING=1 CELLPADDING=3>
  ...
  <%for(int j2=scell;j2<=ecell;j2++){
	...
	%><td align="center" class=FloorC width="1%"><font color="#ffffff"><div id=A<%=String.valueOf(java.lang.Math.abs(i))+1 %>w1><%=i%></div></font></td>
	 ...
	 <td align="center"  <%=bg %>>
	 <% if (!state.equals("")){ %>
	 <a href="#" title="退款登记" onclick="opennewwin('yingshi_t.jsp','?contractno=<%=contractno %>&cusno=<%=customer %>&loft=<%=SecName+loft %>&ldroomcode=<%=roomcode %>&checkQX=<%=request.getParameter("checkQX") %>',690,570,50,50);"><%=RoomNo%></a>
	 <%}else{ %>&nbsp;<%} %>
	 </td> 
	 <%}
	}%>
