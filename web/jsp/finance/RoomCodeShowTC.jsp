...
<body topmargin="1"  oncontextmenu="return false">
  ...
  <%    
		...
		%>
		<script>
        alert("此栋还没有进行基础资料录入操作");
        
</script>
		 <%}else{%>
<form name=frm method="post"  action="RoomCodeShowTC.jsp">
...
<P align="center"><font size=4><b>退款审核</b></font></P>
 ...
 <table width="99%" border="0" CELLSPACING=1 CELLPADDING=3>
  ...
  <%for(int j2=scell;j2<=ecell;j2++){
	...
	%><td align="center" class=FloorC width="1%"><font color="#ffffff"><div id=A<%=String.valueOf(java.lang.Math.abs(i))+1 %>w1><%=i%></div></font></td>
	 ...
	 <td align="center"  <%=bg %>>
	 <% if (!state.equals("")){ %>
	 <a href="#" title="退款审核" onclick="openwin('F_checkFactmoney.jsp?contractno=<%=contractno %>&cusno=<%=customer %>&loft=<%=SecName+loft %>&ldroomcode=<%=roomcode %>&checkQX=<%=request.getParameter("checkQX") %>',690,570,50,50);"><%=RoomNo.substring(RoomNo.indexOf("-")+1)%></a>
	 <%}else{ %>&nbsp;<%} %>
	 </td> 
	 <%}
	}%>
