...
<tr  valign="top">
    <td class="<%if(sel.equals("1")){out.print("menuselect3");}else{out.print("selemenu4");}%>" align="center" width=94 height=21>
	  <a href="#" onclick="document.f1.sel.value=1;findSub('yingshi_t.jsp');"><font color='<%if(sel.equals("1")){out.print("#FFFFFF");}else{out.print("#000000");}%>'>房款管理</font></a>
	</td>
	<td class="<%if(sel.equals("2")){out.print("menuselect3");}else{out.print("selemenu4");}%>" align="center" width=94 height=21>
	 <a href="#" onclick="document.f1.sel.value=2;findSub('yingshi_t.jsp');"><font color='<%if(sel.equals("2")){out.print("#FFFFFF");}else{out.print("#000000");}%>'>税/配套费</font></a>
	</td>
	<td class="<%if(sel.equals("3")){out.print("menuselect3");}else{out.print("selemenu4");}%>" align="center" width=94 height=21>
	 <a href="#" onclick="document.f1.sel.value=3;findSub('yingshi_t.jsp');"><font color='<%if(sel.equals("3")){out.print("#FFFFFF");}else{out.print("#000000");}%>'>票据管理</font></a>
	</td>
...
%>
	    ...
	    <tr  class=listcontent id=TR<%=i%> align="center"     onclick="setpara( '<%= qs %>' , '<%= qianKuan+demurrage%>' , '<%= shouldmoney %>','<%=ysid %>','<%=shouldDate %>','<%=statevalue %>' );changeclass1(this,'clickbg','listcontent');" 　onmouseover="mout(this);">
         ...
         <td><%= fa.getFloat2(ciQiYiJiao,2) %></td>
          ...
          <td><%if (state.equals("2")){out.print(skstate+"&nbsp;&nbsp;");}else{ %><a href="#d"  onclick="dengji(<%=fa.getFloat3(shouldmoney,3) %>,<%=qs %>);">收款</a><%} %><%if (Cortrol.indexOf("F")>=0){
 ...
 <p align=center>
  <b><font size="3" color="#000080">实收一览表</font></b>
	  ...
	  <%if (Cortrol.indexOf("A")>=0){%>
	   <input type=button name=facm value="退款登记" onclick="outdengji();">
...
%>
        <tr class=listcontent align=center  title="<%=context %>">
		 ...
		 <td><%= print_date %></td>
		  ...
		  <%} %>
		  <%}else{ if (Cortrol.indexOf("Q")>=0){%> <img src="../images/edit.gif" alt='修改收款' onclick="Edit('<%= cusno %>','<%= contractno %>','<%= period %>','<%= rowID%>','<%=FactCurry%>','<%=FactCurryRate %>','<%=factmoney %>')" id=bian<%= i2 %> onmouseover="javascript:this.style.cursor='hand'"><%} %>
		 &nbsp;<font color="red">已审核</font><%} %>
		   ...
		   <img src="../images/icon_print.gif" alt='打印收据单'  onclick="printSJ('<%= cusno %>','<%= contractno %>','<%= period %>','<%= rowID%>')" id=bian<%= i2 %> onmouseover="javascript:this.style.cursor='hand'">
		      <%}else{ %>
             <img src="../images/icon_print.gif" alt='打印收据单'  onclick="printSJt('<%= cusno %>','<%= contractno %>','<%= period %>','<%= rowID%>')" id=bian<%= i2 %> onmouseover="javascript:this.style.cursor='hand'">
			  <%} %>
		   <%} %>
    ...
    %>
		 <tr bgcolor="#EBEBEB" align="center">
     <td colspan=2>
    <b>合&nbsp;&nbsp;计:</td><td colspan=7 align="left"> <b>￥<%=fa.getFloat2(sumfactmoney,2)%> </b></td> 
     <td > 
	...
	<tr  class=listtitle  align="center"><td>按揭银行</td><td>放款日期</td><td>放款金额</td><td>登记人</td><td>审核</td><td> </td></tr> 
	...
	while (rs.next()){%>
	  ...
	  <td><%=rs.getString("factmonye") %></td>
	   ...
	   out.print(getbyte("<font color='red'>"+rs.getString("chekcer"))+"&nbsp;"+ rs.getDate("checkdate").toString()+"</font>");
	    ...
	    <td width="10%"> <%if (Cortrol.indexOf("D")>=0){%><img src="../images/button_sc.gif"  border="0"   onmouseover="javascript:this.style.cursor='hand'"  onclick="if (confirm('确认删除吗？'))openwin('deleteAj.jsp?id= <%=ajid%>',10,10,10,10);"> <%}if (Cortrol.indexOf("E")>=0){%>&nbsp;&nbsp; <img src="../images/edit.gif" alt='修改收款'  onclick="openwin('AjEdit.jsp?code= <%=contractno%>',400,350,100,100);"><%} %></td>
...
int cancel=0; 
 ...
 while (rs.next()){
  ...
  <td width="10%"> <%if (Cortrol.indexOf("D")>=0){%>
		  ...
		  <%} %>
		  <%if (Cortrol.indexOf("E")>=0){%><img src="../images/edit.gif" border="0"  onmouseover="javascript:this.style.cursor='hand'" alt='修改' onclick="openwin('ModiTax.jsp?id=<%=id %>&taxmoneys=<%=TaxMoney %>&name=<%=TaxName %>&taxname_type=<%=taxname_type %>&state=<%=state %>',300,300,100,100);" ><%} %>
   ...
   String no=getbyte(rs.getString("no"));
    ...
    <tr class=listcontent id=TR1<%=i%>  align="center"    onclick="changeclass1(this,'clickbg','listcontent');">
	 ...
	 <td width="10%"> <%if (Cortrol.indexOf("D")>=0){%>
		  ...
		  <% 
	 if (!dscheck){
	   if (Cortrol.indexOf("E")>=0){%> <img src="../images/edit.gif" border="0"  onmouseover="javascript:this.style.cursor='hand'" alt='修改'  onclick="openwin('ModiTaxDs.jsp?JKdate=<%=JKdate %>&id=<%=id %>&taxmoneys=<%=TaxMoney %>&name=<%=taxname %>&taxname_type=<%=taxname_type %>',300,300,100,100);" >
	   
	   <%}
	 } else{ if (Cortrol.indexOf("Q")>=0){%><img src="../images/edit.gif" border="0"  onmouseover="javascript:this.style.cursor='hand'" alt='修改'  onclick="openwin('ModiTaxDs.jsp?JKdate=<%=JKdate %>&id=<%=id %>&taxmoneys=<%=TaxMoney %>&name=<%=taxname %>&taxname_type=<%=taxname_type %>',300,300,100,100);" ><%}out.print("<font color=red>已审核</font>") ;}
	    %>
		  </td>
    </tr>
    <%
   ...
   <table width="100%"  align="center" border="0" cellspacing="1"   cellpadding="1"  >
	...
	%>	   
	   <tr  class=listcontent  align="center">
		...
		%>
		  <td><%=fpstate %></td>
		<td><a href="#" onclick="fpmodi('<%=getbyte(rs.getString("code")) %>');">修改</a>&nbsp;<%if (!fpstate.equals("作废")){ %><a href="#" onclick="fpzf('<%=getbyte(rs.getString("code")) %>');">作废</a><%} %><%if (fpstate.equals("作废")){ %><a href="#" onclick="fpff();">恢复</a><%} %> 
      <%if (fpstate.equals("正常")){ %>
	   <img src="../images/icon_print.gif" alt='打印收据单' onclick="printfp('<%= cusno %>','<%= contractno %>','','')" id=bian<%= i2 %> onmouseover="javascript:this.style.cursor='hand'">
		<%} %>
		</td>
