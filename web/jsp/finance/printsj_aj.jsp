...
<input type="submit" value="如果下载数据不成功或者长时间停顿，请重新点击这个按钮尝试..." onclick="execute();">
   ...
   window.location="<%=session.getAttribute("reportAddr")%>printSJaj.rpt";
</script>
  ...
  String ldSQL="select F.FactMonye  factmoney,F.FactDate,C.cus_name from FactAjCharges F,customer C where C.serialno=F.CUSTOMER and F.contractno='"+contractno+"'";
  ...
  try{
   ...
   while (rs.next()){
     factmo+=rs.getInt("factmoney");
	 factdate=getbyte(rs.getString("FactDate"));
	  cusName=getbyte(rs.getString("cus_name"));
   }
     
   rs.close();
   if (!factdate.equals(""))factdate=factdate.substring(0,10);
    SKmoney=String.valueOf(factmo);
 context="收到按揭款购房款";
...
for (int i=0;i<upperMoney.length();i++){
  ...
  if (ldrate.equals("")) 	   ldrate="1";
   ...
   rs.close();
	String sjcontext="insert into printsksj(customer,payment,context,money1"
	  +",money2,rate,curr,txt,checker,r1,r2,r3,r4,r5,r6,r7,r8,r9,r0,sj_date) values('"+cusName+"','"+PayType+"','"+context+"("+loft+")"
	  +"','"+upperMoney+"',"+SKmoney+","+ldrate+",'"+ldcurry+"',"
	  +"'"+BillType+"号:"+ldbillnum+"','"+(String)session.getAttribute("loginname")+"'"
	  +",'"+p1+"','"+p2+"','"+p3+"','"+p4+"','"+p5+"','"+p6+"','"+p7+"','"+p8+"','"+p9+"','"+p10+"','"+factdate+"')";
  ...
  }catch(Exception s){out.print(s+"ok");}
