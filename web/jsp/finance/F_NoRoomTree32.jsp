...
<%
 ...
 <td id="buil_<%=Loft%>"   onclick="changeclassname(this,'BL2','L2');">
  <img src="../images/document1.gif" border=0 align="absmiddle" hspace="4" ><a href="F_NoGetList.jsp?SecNo=<%=SectionSerialNo %>&Loft=<%=Loft %>&SecName=<%=SectionName%>" target="frmright"  title='单击后在右边查看付款情况' onclick="aad2(s<%=num %>,200);"><%=Loft %></a><img src="../images/h.gif"  name="s<%=num %>"  value="<%=num %>" >
  </td>
  </tr>
   <% 
 ...
 <img src="../images/tree.gif" border=0 align="absmiddle" hspace="4" ><a href="F_NoGetList.jsp?SecNo=<%=SectionSerialNo %>&Loft=<%=Loft %>&SecName=<%=SectionName%>&Build=<%=Build %>" target="frmright"  title='单击后在右边查看付款情况' onclick="aad2(s<%=num %>,200);"><%=Build %></a><img src="../images/h.gif"  name="s<%=num %>"  value="<%=num %>" >
...
<%}rsuptown.close();%>
 ...
 <br>
搜索姓氏:<br>
 <select name=speedcusname onchange="searchcus();" style="width=83;" title="选择姓氏后会自动搜索">
 <option><option>
<%
...
<br>
  <input type=text size=10 name=findx title="手工录入搜索信息后再点击搜索按钮"  onKeyDown="key(document.cus.dd)">
  <input type=button name=dd value="搜索" onclick="searchcus1();">
<br>
 </div>
</form>
<%//}%>
 ...
 <script language="JavaScript">
	...
	parent.frmright.document.forms[0].SecNo.value=document.cus.secname1.value;
	if (parent.frmright.document.frm.action=="")
	parent.frmright.document.forms[0].action="FindList.jsp";
