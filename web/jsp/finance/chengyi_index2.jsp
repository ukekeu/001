...
//排序方式
	if (!orderbytype.equals("")&&!orderby.equals("")){
	  ...
	  if (orderbytype.equals("1")&&orderby.equals("1")) orderby=" order by cus_name asc";
	    if (orderbytype.equals("2")&&orderby.equals("0")) {orderby=" order by vipno desc";}
	  if (orderbytype.equals("2")&&orderby.equals("1")) orderby=" order by vipno asc";
	}else{
	orderby=" order by  VIPNO desc,dengji_riqi desc";
...
else findStr+=" and jinger>0 ";
String sql = "" ;
  List list1=new LinkedList();//存放铺位编码
	List list2=new LinkedList();//存放铺位状态
 
	String sqlArr="select   o.customer,convert(char(10),o.date,120) date from order_contract o  where o.state<>3 and o.section in";
		   sqlArr=sqlArr+"(";
		   if (!getbyte1(request.getParameter("section")).equals(""))
		    sqlArr=sqlArr+"'"+getbyte1(request.getParameter("section"))+"')";
			else 
		   sqlArr=sqlArr+ SectionS+")";
		   ...
		   while (rsArr.next()){
			...
			list1.add(getbyte(rsArr.getString("customer")));
			   list2.add(rsArr.getString("date"));
			
		   } 
	      rsArr.close();
 
 
    sql =  "select Top "+pageSize+" * from View_Chenyi where jinger-tk-tk1=0 and aserialNo not in (Select top "+prenum +" aserialNo from  View_Chenyi  where jinger-tk-tk1=0 and zto="+state+" "+ findStr + orderby+") and zto="+state+ findStr +" and section in ("+SectionS+")"+ orderby;
		
    sqlcount = "select count(*) from View_Chenyi where jinger-tk-tk1=0 and zto=" + state
...
<%  
    ...
    <%@ include file="../public_js/qx_checke.jsp"%>
	 <%
//权限审核
String Cortrol=checke_qx(ViewBean,(String)session.getAttribute("loginid"),"F003");
    if (Cortrol==null)Cortrol="";
    %>
	<td><input name="choos" type="radio" value=1 <%if (choos.equals("1"))out.print("checked"); %> onclick="zh();">已收诚意金<input name="choos" type="radio" value=2 <%if (choos.equals("2"))out.print("checked"); %> onclick="zh2();">已退诚意金<input name="choos" type="radio" value=0 onclick="zh1();" <%if (choos.equals("0"))out.print("checked"); %>>未收诚意金
</td>
<% 
    if (Cortrol.indexOf("A")>=0||Cortrol.indexOf("E")>=0){ %> 
   <td>
   <input type=button name=modi value="收款/退款" title="修改诚意客户诚意资料" onclick="refEdit_i();">
   </td>
                ...
                <tr> 
                  <td width="94%"> <table width="100%" cellpadding class="L2" cellspacing="1">
                      <tr class=listtitle> 
                        ...
                        <td align="center"><font color="#FFFFFF">已收诚意金</font></td>
                        <td align="center"><font color="#FFFFFF">收款日期</font></td>
						 <td align="center"><font color="#FFFFFF">已退诚意金</font></td>
                        <td align="center"><font color="#FFFFFF">退款日期</font></td>
                       <%if (state.equals("1")){ %>
                        <td bgcolor="#6E91D5" align="center"><font color="#FFFFFF">认购日期</font></td>
                      ...
                      <%// out.print(sql);
                            ...
                            ResultSet rsB = fBean.executeQuery( sql ) ;
                              while( rsB.next() ) {
                                ...
                                Timestamp REF_DATE = rsB.getTimestamp( "sk_date" ) ;
								Timestamp REF_DATE2 = rsB.getTimestamp( "tk_date" ) ;
								String tk= rsB.getString( "tk" ) ;
                      ...
                      <tr id=TR<%= i %> align=center class="listcontent" onmouseover="changCur(this)" onclick="savepara('<%= REF_ID %>','<%=REF_name %>','<%=REF_IF_ID_CARD %>','<%=REF_TYPE %>','<%=REF_SOURCE %>');changeclass1(this,'clickbg','listcontent');" ondblclick="window.open('chengyi_edit.jsp?ref_id=<%=REF_ID %>','edit_i','height=550,width=550,left=150,top=10,resizable=yes,scrollbars=yes,toolbar=no,location=no,directories=no,status=no,menubar=no');"> 
                       ...
                       <%if (state.equals("1")){ %>
					    <td ><% 
						if (list1.contains(cusno)){
		   int index=list1.indexOf(cusno);
		   out.print((String)list2.get(index));
						}
						 %></td>
						<%} %>
                        <td ><%= REF_IF_ACC %></td>
					 ...
					 <!--
						<%if (Cortrol.indexOf("E")>=0){ %>
						  ...
						  <td ><img src="../images/icon_print.gif" alt='打印诚意收据' onclick="openwin('printcy.jsp?money=<%=REF_SOURCE %>&REF_ID=<%=REF_ID %>');" onmouseover="javascript:this.style.cursor='hand'"></td>
                      </td>
...
function refEdit_i()
{
    ...
    winR = window.open("chengyi_edit.jsp?ref_id=" + ref_id ,"edit_i","height=550,width=540,left=150,top=10,resizable=yes,scrollbars=yes,toolbar=no,location=no,directories=no,status=no,menubar=no");
}
...
document.form1.action="chengyi_index.jsp1?REF_ID="+REF_ID+"&chstate="+para;
document.form1.submit();
}
}
  function outexcel(){
    if (document.form1.section.value==""){alert("请选择项目");return false;}
	  window.open("chengyi_excel.jsp?section="+document.form1.section.value)
  
  }
  function zh(){
    document.form1.action="chengyi_index1.jsp?choos=1";
    document.form1.submit();
  }
   function zh1(){
    document.form1.action="chengyi_index.jsp?";
    document.form1.submit();
  }
    function zh2(){
     document.form1.action="chengyi_index2.jsp?";
     document.form1.submit();
  }
</script>
