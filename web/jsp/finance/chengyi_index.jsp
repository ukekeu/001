...
if (choos.equals("0"))findStr+=" and jinger-tk-tk1=0 ";
else findStr+=" and jinger-tk-tk1>0 ";
String sql = "" ;
   List list1=new LinkedList();//存放铺位编码
	...
	String sqlArr="select   o.customer,convert(char(10),o.date,120) date from order_contract o  where o.state<>3 and o.section in";
		   ...
		   while (rsArr.next()){
			...
			list1.add(rsArr.getString("customer"));
...
boolean allSecNo = true ;
    ...
    sql =  "select Top "+pageSize+" * from View_Chenyi where jinger=0 and  aserialNo not in (Select top "+prenum +" aserialNo from  View_Chenyi  where   jinger=0 and  zto="+state+" "+ findStr + orderby+") and zto="+state+ findStr +" and section in ("+SectionS+")"+ orderby;
		
    sqlcount = "select count(*) from View_Chenyi where  jinger=0 and  zto=" + state
                ...
                <tr> 
                  <td width="94%"> <table width="100%" cellpadding class="L2" cellspacing="1">
                      ...
                      <tr id=TR<%= i %> align=center class="listcontent" onmouseover="changCur(this)" onclick="savepara('<%= REF_ID %>','<%=REF_name %>','<%=REF_IF_ID_CARD %>','<%=REF_TYPE %>','<%=REF_SOURCE %>');changeclass1(this,'clickbg','listcontent');" ondblclick="window.open('chengyi_edit.jsp?ref_id=<%=REF_ID %>','edit_i','height=550,width=550,left=150,top=10,resizable=yes,scrollbars=yes,toolbar=no,location=no,directories=no,status=no,menubar=no');"> 
                        ...
                        <td ><%= VIPNO %></td>
					    <td  align="left"><%if (TK>0){out.println("<font color=red>");}%><%= Repalce(REF_name,"/") %><%//if (zto.equals("1")){out.print("("+ new_name+" "+new_room+" "+new_room_date+")");} %><%if (TK>0){out.println("</font>");}%></td>
                        <td ><%= REF_TITLE %></td>
...
function refEdit_i()
{
    ...
    winR = window.open("chengyi_edit.jsp?ref_id=" + ref_id ,"edit_i","height=550,width=560,left=150,top=10,resizable=yes,scrollbars=yes,toolbar=no,location=no,directories=no,status=no,menubar=no");
}
