...
String orderSQL="SELECT LOFT+ROOM_NO ROOM  ,CUSTOMER FROM ORDER_CONTRACT where state=2 and section in("+SectionS+")"+strWhere;
 ...
 sql="select Top "+pageSize+"   customer.Code,customer.serialno,  customer.ID_Card, customer.cus_name,customer.cus_sex, customer.Phone,customer.sj,customer.Address,customer.Zip_Code ,customer.sj,customer.Address,customer.Zip_Code ,Order_Contract.seller,Order_Contract.floors,convert(char(24),customer.visit_date,111) as   visit_date,customer.bbj,customer.sj,Order_Contract.VisaDate,Order_Contract.sectionname,Order_Contract.section,Order_Contract.loft,Order_Contract.room_no,Order_Contract.ContractNo,Order_Contract.yong_tu from customer,Order_Contract where  Order_Contract.customer=customer.serialno and  Order_Contract.Section in("+SectionS+") and  customer.Code not in (Select top "+prenum +" customer.code from customer,Order_Contract where  Order_Contract.customer=customer.serialno and Order_Contract.Section in("+SectionS+") and customer.member=0 and Order_Contract.state=2" +strWhere +orderby+")  and customer.member=0 and  Order_Contract.state=2 " + strWhere;

 sqlcount="SELECT count(customer.code) code from customer,Order_Contract where  Order_Contract.customer=customer.serialno and customer.member=0 and  Order_Contract.Section in("+SectionS+")  and  Order_Contract.state=2 "+strWhere;
...
<%  
   ...
   <option value=cus_name>客户姓名</option>
    <option value=loft+room_no>购买单位</option>
   ...
   <input type=text name=search_like size=30 title="模糊查询，可查询姓名、电话、地址，双击可弹出日期" ondblclick="fPopUpCalendarDlg(search_like);return false;" value="<%=search_like %>"><input type=button name=fin value="查 询"  onclick="document.form1.submit();" >&nbsp;<input type=button name=outdata value="导入会员库" onclick="outexcel();"></td>  </tr></table>
...
<table BORDER=0 width="110%"  id=AutoNumber  CELLSPACING=1 CELLPADDING=1 ALIGN=center >
  <tr  class=listtitle>
      <td height="25"  align="center"><font color=white>序号</font></td>
  <td  align="center"><font color=white>客户姓名</font></td>
  <td  align="center"><font color=white>身份证号</font></td>
  <td  align="center"><font color=white>联系电话</font></td>
 <td  align="center"><font color=white>联系手机</font></td>
  ...
  <td  align="center"><font color=white>签约日期</font></td>
    <td  align="center"><font color=white>用途</font></td>
...
int tr=0;
  ...
  %>
   ...
   <tr id=TR<%=i %>   trid=tr<%=tr %> align="center" class=listcontent  onmouseover="mout(this);"  onclick="savepara('<%=no%>'); changeclass1(this,'clickbg','listcontent');" > 
	<td  align="center"><input type="checkbox" name=choose value="<%=no%>"><%=i %>　</td>
...
<script>
  ...
  function outexcel(){
    ...
    if (document.form1.section.value==""){alert("请选择项目");return false;}
	 if (getseldocs2()==""){alert("请选择业主");return false;}
	  window.open("member.jsp?cusno="+getseldocs2());
